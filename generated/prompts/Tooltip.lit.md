...
- `trigger`: slot, the default `<slot>`, required
- `popup`: element
...
notes: 'Wraps the slotted trigger; because aria-describedby cannot cross the shadow
  ...
  is the visible copy. ID references do not cross into a trigger''s shadow root (ds-button''s
  real <button> is inside it), so for a custom-element trigger Tooltip passes the
  text instead: `describes: false` sets `aria-label` to `content` on the trigger host,
  `describes: true` sets `aria-description`, and system triggers (ds-button, ds-link,
  ds-input) forward both to their inner control; a plain light-DOM trigger gets aria-describedby/aria-labelledby.
  The host is `display: contents` and carries `data-ds`; the bubble carries `data-part="popup"`.
  A custom-element trigger is one whose tag name contains a hyphen, whether or not
  it is upgraded or its shadow root is open. Unlike web, the bubble stays mounted
  while hidden (popover closed, aria-hidden), so the `exit` fade plays and slotchange
  does not re-fire on every show. `offset` is read in px the same way as on web, through
  a hidden probe''s `padding-left`, and a duration through its `transition-duration`;
  the probe is the always-present visually-hidden node rather than an appended element,
  which would re-fire slotchange on the host. A trigger''s focusability is not readable
  through a custom-element host (ds-button''s host is tabindex -1 with delegatesFocus),
  so the non-focusable-child development warning fires only for a plain element whose
  tabindex is below 0; a hyphenated tag that is not focusable is not warned about.'
```
