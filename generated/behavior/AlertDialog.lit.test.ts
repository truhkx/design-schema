...
async function setup(given: Record<string, unknown> = {}) {
  ...
  const s = {
    ...
    props,
    root_: () => el,
    scrim: () => ((el.matches('[role="alertdialog"]') ? el : null) ?? deep(root, '[role="alertdialog"]') ?? (el.matches('[part~="scrim"], [data-part="scrim"]') ? el : null) ?? deep(root, '[part="scrim"]') ?? deep(root, '[data-part="scrim"]') ?? root.firstElementChild) as HTMLElement,
    cancelButton: () => ((el.matches('[part~="cancelButton"], [data-part="cancelButton"]') ? el : null) ?? deep(root, '[part="cancelButton"]') ?? deep(root, '[data-part="cancelButton"]')) as HTMLElement,
    confirmButton: () => ((el.matches('[part~="confirmButton"], [data-part="confirmButton"]') ? el : null) ?? deep(root, '[part="confirmButton"]') ?? deep(root, '[data-part="confirmButton"]')) as HTMLElement,
  };
  return s;
}
...
describe('ds-alert-dialog', () => {
  test('confirm-button-fires-on-confirm', async () => {
    ...
    await userEvent.click(...(await pointerAt(activatable(s.confirmButton(), s.el), s.confirmButton())));
    expect(s.events.onConfirm).toHaveBeenCalled();
  });
  test('cancel-button-fires-on-cancel', async () => {
    const s = await setup({"open": true});
    await userEvent.click(...(await pointerAt(activatable(s.cancelButton(), s.el), s.cancelButton())));
    expect(s.events.onCancel).toHaveBeenCalled();
  });
  test('focus-starts-on-the-cancel-button', async () => {
    const s = await setup({"open": true});
    expect(activeChain()).toContain(focusedPart(s.cancelButton(), s.el));
  });
  ...
  test('the-cancel-button-is-named-from-copy', async () => {
    const s = await setup({"open": true});
    expect(flatText(s.el)).toMatch(new RegExp("Cancel"));
  });
  test('renders', async () => {
    const s = await setup({"open": true});
  ...
  test('has-accessible-name', async () => {
    const s = await setup({"open": true});
    if (hostName(s.el) !== null) expect(hostName(s.el)).toBe(s.props.heading);
    else expect(s.scrim()).toHaveAccessibleName(s.props.heading);
  });
