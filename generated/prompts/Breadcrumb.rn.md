...
notes: "A horizontal, wrapping View with role=\"navigation\" (semantic on react-native-web;\
  ...
  \ is Text with accessibilityState={{ selected: true }} and accessibilityLabel '<label>,\
  \ <copy.current>' (screen readers would otherwise say only \"selected\"). Separators\
  \ are Text with importantForAccessibility=\"no\" / accessibilityElementsHidden.\
  \ The one root View is both the `nav` and `list` parts (testID `Breadcrumb`; there\
  \ is no `Breadcrumb.nav` or `Breadcrumb.list`); items are Views with testID `Breadcrumb.item`,\
  \ the Text wrapping each Link carries `Breadcrumb.link` (Link takes no testID),\
