...
notes: The label is a real <label for=id>. Description and error are linked with aria-describedby;
  the error element has role="alert". The forwarded ref targets the <input> (so focus()
  and select() work), not the root wrapper. validity follows the full precedence through
  setCustomValidity with the copy message (cleared when the field passes or is disabled);
  because a custom error makes validity.valid false, the browser/type step reads the
  specific flags (typeMismatch, badInput, patternMismatch, range, step, length). `readOnly`
  is not a schema prop but passes through the native input props; `disabled` forces
  it on.
```
