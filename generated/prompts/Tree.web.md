...
notes: "A <div data-ds=\"Tree\" data-part=\"container\"> holding the Heading when\
  ...
  \ for 500 ms. Pointer: click on a node focuses it and does what Space does (select\
  \ in single, toggle in multiple); double-click does what Enter does, and a click\
  \ whose `detail` is 2 or more does not toggle again (a double-click toggles once,\
  \ then activates); click on the chevron toggles expansion and focuses the node without\
  \ changing selection. Lazy placeholder: a <li role=\"treeitem\" aria-disabled=\"\
  true\"> holding copy.loading inside the group, not navigable, with no data-part,\
  \ and aria-busy on the parent treeitem. Empty `nodes`: the <ul> renders with no\
  \ items and copy.empty is a `Text tone=\"muted\"` below it inside the container."
```
