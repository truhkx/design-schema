...
- prop: dragToDismiss
  attribute: no-drag-to-dismiss
notes: "Shadow <dialog> with showModal(), laid out as web: the surface at the bottom\
  \ edge, capped at the maxWidth token and centred, at every width \u2014 no matchMedia\
  \ and no `<ds-dialog>`. `close` and `drag-dismiss` are composed CustomEvents. The\
  \ footer part is rendered only when the `footer` slot has assigned children, so\
  \ a sheet with no footer gets no empty footer part. Every composed part (heading,\
  \ closeButton, body, footer) is a sheet-owned wrapper element carrying `data-part`,\
  \ holding the `<ds-heading>`, `<ds-button>`, `<ds-box>` or `<ds-stack>`: ds-box\
  \ overwrites its own data-part with `surface` and the other ds-* hosts carry none.\
  \ Accessible name: the shadow <dialog> carries aria-label from the heading text,\
