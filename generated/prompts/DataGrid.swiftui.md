...
- `columnWidth`: token `space.20`; computed `theme.space20 * 2`
...
notes: 'Tablets and Catalyst first, as on RN. A `ScrollView([.horizontal, .vertical])`
  ...
  each cell is an accessibility element labelled copy.cellLabel with `.accessibilityValue(copy.position)`;
