...
notes: "A native <input type=\"checkbox\"> styled with appearance: none \u2014 never\
  ...
  \ element, as in Input. A click whose target is the row itself (the gap) is forwarded\
  \ to the input. Fieldset's group `disabled` arrives as the `disabled` prop Fieldset\
  \ passes to its direct child fields; there is no React FieldsetContext. copy.checked,\
  \ copy.unchecked and copy.mixed are not used (native checked state plus aria-checked=mixed)."
```
