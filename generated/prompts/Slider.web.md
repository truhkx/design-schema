...
notes: Custom thumbs (<div role="slider" tabindex="0">) on a track rather than <input
  ...
  formatValue. A hidden <input name> (two with the same name for a range) carries
  the decimal string(s) for native forms. The label, value, description and error
  Texts carry their own data-part, as the composition table says (no wrapper takes
  the part name); the error Text sits inside an unparted `<div role="alert">`, as
  on Lit. `copy.pageUpAction`, `pageDownAction`, `homeAction` and `endAction` are
