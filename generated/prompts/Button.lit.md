...
notes: "Wraps a native <button> in the shadow root with delegatesFocus so the host\
  ...
  \ and part names camelCase. A blocked activation (disabled or loading) is swallowed\
  \ on the inner button with preventDefault() and stopPropagation(): neither `press`\
