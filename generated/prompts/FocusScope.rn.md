...
notes: "There is no Tab order to confine on native. `trapped && active` maps to accessibilityViewIsModal\
  ...
  \ div; the composing overlay's own dialog carries aria-modal), detected with `Platform.OS\
  \ === 'web'`; the Jest suite runs under the preset's `ios` OS, so no behavior scenario\
  \ covers that branch. There is no way to walk children, so the development warning\
  \ the web notes require for a trapped scope with no focusable descendants is not\
  \ emitted here: the one case where a trapped native scope is as inescapable as a\
  \ web one goes unreported, a stated limit of having no focusable-descendant walker.\
  \ The `scope` part is the root and carries the bare testID \"FocusScope\". onEscapeAttempt\
  \ never fires on this platform: nothing can attempt to leave a Tab order that does\
  \ not exist. Hardware-keyboard Tab wrapping is not implemented; that is a platform\
  \ limit, so the two wrap keyboard rules exclude rn (on react-native-web Tab is not\
