...
notes: "A <div role=\"radiogroup\" aria-label> of <button role=\"radio\" aria-checked\
  ...
  \ `iconOnly` segments carry `aria-label` = the option label themselves and are wrapped\
  \ in Tooltip with `content` = the label, `describes: false`, and default placement\
  \ and delay (Tooltip's warm window already makes moving along the control instant)."
```
