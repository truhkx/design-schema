...
notes: "A FlatList with fixed getItemLayout (virtualized for free); the FlatList is\
  ...
  \ no announcement here. FlatList also gives the set of body rows no wrapper of its\
  \ own, so the `body` rowgroup part has no element on native: the list is the grid,\
  \ and only the header row carries a rowgroup. This is the one component where a\
  \ phone is a poor fit; the doc recommends Table with `responsive: stack` for phone-first\
  \ screens."
```
