...
- Icon: the lit platform note sets `:host([inline])` to inline-block with font-size inherit only, while the Guidance's Lit section adds `vertical-align: -0.125em; inline-size: 1em; block-size: 1em`. I kept the fuller Guidance version (it matches the styles binding's 'inline ones sit at vertical-align -0.125em'); the platform note should list all of them.
