...
async function setup(given: Record<string, unknown> = {}) {
  ...
  const s = {
    ...
    props,
    root_: () => el,
    trigger: () => ((el.matches('[role="dialog"]') ? el : null) ?? deep(root, '[role="dialog"]') ?? (el.matches('[part~="trigger"], [data-part="trigger"]') ? el : null) ?? deep(root, '[part="trigger"]') ?? deep(root, '[data-part="trigger"]') ?? root.firstElementChild) as HTMLElement,
...
describe('ds-popover', () => {
  ...
  test('the-panel-is-named-by-its-heading', async () => {
    ...
    if (hostName(s.el) !== null) expect(hostName(s.el)).toBe("Filters");
    else expect(s.trigger()).toHaveAccessibleName("Filters");
  });
  test('renders', async () => {
    const s = await setup({"open": true});
