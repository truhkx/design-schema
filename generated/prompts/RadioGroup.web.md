...
notes: 'A <fieldset role="radiogroup"> with a <legend>, containing native <input type="radio"
  ...
  implicit role is `group`, which is not what this component declares. The radioIndicator
  part is a ::after pseudo-element with no node of its own, so it carries no hook
  on any platform; shadow parts elsewhere are the anatomy names in kebab-case (`radio-label`,
  `radio-description`, `error-message`). Native radios sharing a name already implement
