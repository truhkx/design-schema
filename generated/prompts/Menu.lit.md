...
notes: "Uses the Popover API (popover=\"manual\", showPopover()) for top-layer rendering\
  ...
  \ cannot reach a slotted trigger from the shadow root. `<ds-button>` forwards only\
  \ `expanded` to its inner <button>, and an IDREF cannot cross its shadow root, so\
  ...
  ` and `part=\"popup list\"`. The trigger part is an overlay-owned wrapper element\
  \ carrying `data-part=\"trigger\"` around the ds-button. `anchor` is a property\
  \ (the element), not an attribute. Focus leaving the host, or the window losing\
  \ focus, closes with reason `focus-out`."
```
