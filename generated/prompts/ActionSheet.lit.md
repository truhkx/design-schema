...
notes: '`actions` is a property. Composed `action` (detail { id }) and `close` (detail
  ...
  wide presentation renders <ds-menu>, positioned at the opener''s rect with its own
  trigger hidden, as on web. The heading is named by `aria-label` rather than an id
  reference, since ids do not cross the shadow root.'
```
