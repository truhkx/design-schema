...
notes: 'Wraps a native <a> in the shadow root with delegatesFocus. No custom event:
  ...
  anchor. An empty `href` still renders `href=""` on the anchor rather than omitting
  the attribute, which would make it a non-link and drop it from the focus order.
  The tone colour rules are scoped `:host(:not([tone=''inherit'']))`, not `:host([tone=''default''])`,
  so a link is coloured before the attribute reflects and for any value that is not
  `inherit`. `:host([hidden]) { display: none }` overrides the inline default, as
