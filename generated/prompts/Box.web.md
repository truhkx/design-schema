...
notes: 'A plain element with classes for each enum value; `surface: none` sets no
  background. insetBlock/insetInline modifiers win over inset. No margin, ever. The
  root is the `surface` part and carries `data-part="surface"`, written before `...rest`
  so a composing parent can relabel it (Popover and BottomSheet pass `data-part="body"`).
  ...
  it a layout-only class. Props are typed against `div` for every `element` value;
  Box is not polymorphic.'
```
