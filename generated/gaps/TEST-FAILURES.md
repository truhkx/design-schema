# Test failures after the harness fixes (2026-09-10T02:20)
...
## Lit — 8 failing, 486 passing, 2 skipped
...
| control-is-focusable | `packages/lit/src/AlertDialog.test.ts` | AssertionError: expected <ds-alert-dialog …(3)></ds-alert-dialog> to be <ds-button …(7)></ds-button> // Object.is equality | Focus did not land where the doc says (initial focus / focus-after-action). |
| control-is-focusable | `packages/lit/src/Dialog.test.ts` | AssertionError: expected <ds-dialog …(4)>…(3)</ds-dialog> to be <ds-input …(2)></ds-input> // Object.is equality | Focus did not land where the doc says (initial focus / focus-after-action). |
