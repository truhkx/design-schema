...
component:
  ...
  props:
    ...
    hideCaption:
      type: boolean
      default: false
      description: 'Visually hide the caption; it remains the accessible name. The caption part is the composed Heading element itself, with no wrapper, so hiding it also sends `space.0` in place of `captionGap`: a hidden caption leaves no gap above the header.'
    columns:
      type: array
      required: true
