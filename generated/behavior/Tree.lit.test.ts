...
async function setup(given: Record<string, unknown> = {}) {
  ...
  const s = {
    ...
    props,
    root_: () => el,
    container: () => ((el.matches('[role="tree"]') ? el : null) ?? deep(root, '[role="tree"]') ?? (el.matches('[part~="container"], [data-part="container"]') ? el : null) ?? deep(root, '[part="container"]') ?? deep(root, '[data-part="container"]') ?? root.firstElementChild) as HTMLElement,
    nodeRow: () => ((el.matches('[part~="nodeRow"], [data-part="nodeRow"]') ? el : null) ?? deep(root, '[part="nodeRow"]') ?? deep(root, '[data-part="nodeRow"]')) as HTMLElement,
...
describe('ds-tree', () => {
  test('the-expand-button-expands-a-node', async () => {
    ...
    await userEvent.click(located(activatable(s.expandButton(), s.el)), { force: true });
  ...
  test('clicking-a-node-selects-it', async () => {
    ...
    await userEvent.click(located(activatable(s.nodeRow(), s.el)), { force: true });
  ...
  test('the-empty-message-shows-when-there-are-no-nodes', async () => {
    ...
    expect(flatText(s.el)).toMatch(new RegExp("Nothing\\ here\\."));
  });
  test('renders', async () => {
    const s = await setup({});
