...
notes: "Native <table><thead><tbody> with EVERY role stated explicitly (role=\"table\"\
  ...
  \ stack` below the prose breakpoint: a container query switches tr/td to display\
  \ block/grid, thead is visually hidden (not display none, so the columnheaders remain\
  \ in the tree), and each td gets a ::before from a data-label attribute holding\
  \ the header text \u2014 aria-hidden as text is already associated by the roles.\
  ...
  \ labels the action row like every other cell. Rows are keyed by id; no virtualization\
  \ in this component (that is DataGrid)."
```
