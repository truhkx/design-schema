...
notes: Its own element following ds-data-grid's structure, keyboard handling and CSS
  ...
  strings above are TreeGrid's own, so nothing is imported from DataGrid). `expanded`
  and `selected` as properties; composed `expand-change`, `expand` with bare detail
  values.
```
