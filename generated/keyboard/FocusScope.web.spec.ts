// Generated by tools/keyboard_tests.ts from the `keyboard` block of the FocusScope doc. Do not edit.
...
test.describe('FocusScope (web) keyboard', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/iframe.html?id=focusscope-react--keyboard&viewMode=story');
    await expect(page.locator('[data-ds="FocusScope"]').first()).toBeVisible({ timeout: 15_000 });
  });
  test('Tab: From the last focusable descendant, wraps to the first.', async ({ page }) => {
    const root = await subject(page, page.locator('[data-ds="FocusScope"]').first());
  ...
  test('Shift+Tab: From the first focusable descendant, wraps to the last.', async ({ page }) => {
    const root = await subject(page, page.locator('[data-ds="FocusScope"]').first());
