// Generated by tools/keyboard_tests.ts from the `keyboard` block of the RadioGroup doc. Do not edit.
...
test.describe('RadioGroup (web) keyboard', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/iframe.html?id=radiogroup-react--keyboard&viewMode=story');
    await expect(page.getByRole('radiogroup').first()).toBeVisible({ timeout: 15_000 });
  });
  test.skip('Tab: Moves into the group, to the selected radio (the first when none is selected); from inside, leaves the group — one tab stop. — manual', async () => {});
  test('ArrowDown: Moves to and selects the next enabled radio, wrapping.', async ({ page }) => {
    const root = await subject(page, page.getByRole('radiogroup').first());
  ...
  test('ArrowRight: Moves to and selects the next enabled radio, wrapping.', async ({ page }) => {
    const root = await subject(page, page.getByRole('radiogroup').first());
    await focusAt(page, root, 0);
    const before = await focusIndex(page, root);
    const stateBefore = await ariaState(page);
  ...
  test('ArrowUp: Moves to and selects the previous enabled radio, wrapping.', async ({ page }) => {
    const root = await subject(page, page.getByRole('radiogroup').first());
  ...
  test('ArrowLeft: Moves to and selects the previous enabled radio, wrapping.', async ({ page }) => {
    const root = await subject(page, page.getByRole('radiogroup').first());
    await focusAt(page, root, await focusableCount(page, root) - 1);
    const before = await focusIndex(page, root);
    const stateBefore = await ariaState(page);
  ...
  test.skip(' : Selects the focused radio when the arrows did not already select it. — manual', async () => {});
});
