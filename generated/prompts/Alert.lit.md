...
notes: "The role is a plain `role` attribute on the host (not ElementInternals), set\
  ...
  \ else the slotted body text (a status region is named by its content), updated\
  ...
  \ tone `<ds-icon>`; both children keep their own hooks. The same next-focusable\
  \ rule as web applies on dismiss."
```
