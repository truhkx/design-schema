...
async function setup(given: Record<string, unknown> = {}) {
  ...
  const s = {
    ...
    props,
    root_: () => el,
    trigger: () => ((el.matches('[role="button"]') ? el : null) ?? deep(root, '[role="button"]') ?? (el.matches('[part~="trigger"], [data-part="trigger"]') ? el : null) ?? deep(root, '[part="trigger"]') ?? deep(root, '[data-part="trigger"]') ?? root.firstElementChild) as HTMLElement,
  };
  return s;
}
...
describe('ds-disclosure', () => {
  ...
  test('has-accessible-name', async () => {
    const s = await setup({"open": true});
    if (hostName(s.el) !== null) expect(hostName(s.el)).toBe(s.props.summary);
    else expect(s.trigger()).toHaveAccessibleName(s.props.summary);
  });
