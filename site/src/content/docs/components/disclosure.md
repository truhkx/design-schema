...
component:
  ...
  styles:
    ...
    triggerBackgroundHover: { token: color.background.subtle, part: trigger, state: hover, description: 'Pointer hover and pressed state of the trigger — web and Lit apply it on :hover and :active, RN on onHoverIn/onHoverOut or pressed; suppressed while disabled. The background change is instant (no transition).' }
    ...
    focusRing: { token: color.border.focus, part: trigger }
    focusRingWidth: { token: border.width.focus, part: trigger }
    minTarget: { token: size.target.min, part: trigger }
