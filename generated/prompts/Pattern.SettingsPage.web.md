# Generate pattern: Settings page for React (web)
...
Write `packages/react/demo/SettingsPage.tsx` exporting a function component named `SettingsPage`, plus `packages/react/demo/SettingsPage.stories.tsx` with `title: 'Patterns/SettingsPage'` and a `Default` story. Do not add anything to `src/index.ts`; the page lives in `demo/`, where adopters copy it from.
...
Every one exists in `packages/react/src`; import from there and read a file only when a prop's behaviour is unclear.

- `Landmark` — `packages/react/src/Landmark.*` (header | nav | main | aside | footer | section | form)
- `Container` — `packages/react/src/Container.*` (div)
- `Stack` — `packages/react/src/Stack.*` (div)
- `Heading` — `packages/react/src/Heading.*` (h1–h6)
- `Tabs` — `packages/react/src/Tabs.*` (div)
- `Form` — `packages/react/src/Form.*` (form)
- `Fieldset` — `packages/react/src/Fieldset.*` (fieldset)
- `Input` — `packages/react/src/Input.*` (input)
- `Button` — `packages/react/src/Button.*` (button)
- `Checkbox` — `packages/react/src/Checkbox.*` (input)
- `Switch` — `packages/react/src/Switch.*` (input)
- `RadioGroup` — `packages/react/src/RadioGroup.*` (fieldset)
- `SegmentedControl` — `packages/react/src/SegmentedControl.*` (div)
- `Card` — `packages/react/src/Card.*` (article)
- `Text` — `packages/react/src/Text.*` (p)
- `Alert` — `packages/react/src/Alert.*` (div)
- `Toast` — `packages/react/src/Toast.*` (div)
- `AlertDialog` — `packages/react/src/AlertDialog.*` (dialog)
