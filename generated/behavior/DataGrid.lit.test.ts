...
async function setup(given: Record<string, unknown> = {}) {
  ...
  const s = {
    ...
    props,
    root_: () => el,
    container: () => ((el.matches('[role="grid"]') ? el : null) ?? deep(root, '[role="grid"]') ?? (el.matches('[part~="container"], [data-part="container"]') ? el : null) ?? deep(root, '[part="container"]') ?? deep(root, '[data-part="container"]') ?? root.firstElementChild) as HTMLElement,
...
describe('ds-data-grid', () => {
  ...
  test('a-custom-empty-message-replaces-the-default', async () => {
    ...
    expect(flatText(s.el)).toMatch(new RegExp("No\\ prices\\ loaded\\."));
  });
  test('renders', async () => {
    const s = await setup({});
