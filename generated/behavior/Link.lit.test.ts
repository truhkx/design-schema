...
async function setup(given: Record<string, unknown> = {}) {
  ...
  const s = {
    ...
    props,
    root_: () => el,
    anchor: () => ((el.matches('[role="link"]') ? el : null) ?? deep(root, '[role="link"]') ?? (el.matches('[part~="anchor"], [data-part="anchor"]') ? el : null) ?? deep(root, '[part="anchor"]') ?? deep(root, '[data-part="anchor"]') ?? root.firstElementChild) as HTMLElement,
  };
  return s;
}
...
describe('ds-link', () => {
  test('click-fires-on-press', async () => {
    const s = await setup({});
    await userEvent.click(...(await pointerAt(activatable(s.anchor(), s.el), s.anchor())));
  ...
  test('external-link-announces-that-it-leaves', async () => {
    ...
    expect(flatText(s.el)).toMatch(new RegExp("\\(opens\\ in\\ new\\ tab\\)"));
    if (hostName(s.el) !== null) expect(hostName(s.el)).toBe("View the billing history (opens in new tab)");
    else expect(s.anchor()).toHaveAccessibleName("View the billing history (opens in new tab)");
  });
  ...
  test('has-accessible-name', async () => {
    ...
    else expect(s.anchor()).toHaveAccessibleName(s.props.label);
  });
