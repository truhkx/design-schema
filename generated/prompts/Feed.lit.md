...
notes: '`items` as a property (`content` and `actions` typed as any lit-html renderable);
  ...
  templates; Card labels itself with aria-label from its heading (ids do not cross
  shadow roots), so Feed sets no aria-labelledby. Parts, run order, the always-present
  role="status" new-items row and aria-busy on the host follow the web notes. Ctrl+Home/End
  walk the document for focusables (shadow-piercing, as FocusScope). Composed `load-more`,
  `show-new`, `item-visible`.'
```
