...
async function setup(given: Record<string, unknown> = {}) {
  ...
  const s = {
    ...
    props,
    root_: () => el,
    landmark: () => ((el.matches('[role="searchbox"]') ? el : null) ?? deep(root, '[role="searchbox"]') ?? (el.matches('[part~="landmark"], [data-part="landmark"]') ? el : null) ?? deep(root, '[part="landmark"]') ?? deep(root, '[data-part="landmark"]') ?? root.firstElementChild) as HTMLElement,
    ...
    submitButton: () => ((el.matches('[part~="submitButton"], [data-part="submitButton"]') ? el : null) ?? deep(root, '[part="submitButton"]') ?? deep(root, '[data-part="submitButton"]')) as HTMLElement,
  };
  return s;
}
...
describe('ds-search', () => {
  ...
  test('the-submit-button-submits-the-query', async () => {
    ...
    await userEvent.click(...(await pointerAt(activatable(s.submitButton(), s.el), s.submitButton())));
    expect(s.events.onSubmit).toHaveBeenCalled();
  });
  ...
  test('has-accessible-name', async () => {
    ...
    else expect(s.landmark()).toHaveAccessibleName(s.props.label);
  });
