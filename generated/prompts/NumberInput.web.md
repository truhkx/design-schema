...
notes: "An <input type=\"text\" inputmode=\"decimal\" role=\"spinbutton\"> rather\
  ...
  \ the pointer/click handlers; aria-hidden=\"true\" goes on the element wrapping\
  \ both spans (not on the Buttons), and the Buttons get `disabled` at a bound or\
  \ when the field is disabled. Parsing: \".\" is read as the decimal separator only\
  \ when the locale's decimal separator is absent from the text (so de-DE \"1.234,5\"\
  \ is 1234.5, and \"1.5\" is 1.5). There is no React FieldsetContext: Fieldset passes\
  \ `disabled` to its direct child fields, and NumberInput uses only that prop; the\
  \ legend is not used."
```
