...
component:
  ...
  styles:
    tabColor: { token: color.foreground.muted }
    tabSelectedColor: { token: color.foreground.strong }
    tabHoverBackground: { token: color.background.subtle }
    tabPaddingBlock: { token: space.sm }
    ...
    panelGap: { token: layout.gap.loose, description: Between the tab list and the panel. }
