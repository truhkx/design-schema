...
notes: 'There is no hover on touch, so no tooltip surface is shown by default: `content`
  ...
  On react-native-web the bubble takes `onPointerEnter`/`onPointerLeave` with pointerEvents
  auto (hoverable, 1.4.13); on native it is pointerEvents none. The bubble is hidden
  from accessibility (accessibilityElementsHidden, importantForAccessibility no-hide-descendants):
  the hint or label already carries the text, so there is no visually-hidden copy
  on rn.'
```
