...
import '../../packages/lit/src/Form.js';
import meta from '../../packages/lit/src/Form.stories.js';
...
async function setup(given: Record<string, unknown> = {}) {
  ...
  el.addEventListener('invalid', events.onInvalid as unknown as EventListener);
...
describe('ds-form', () => {
  ...
  test('renders-validate-submit', async () => {
    const s = await setup({"validate": "submit"});
    expect(s.el.shadowRoot ? s.root.childElementCount > 0 : s.el.isConnected).toBe(true);
  });
  test('renders-validate-blur', async () => {
    const s = await setup({"validate": "blur"});
    expect(s.el.shadowRoot ? s.root.childElementCount > 0 : s.el.isConnected).toBe(true);
  });
  test('renders-validate-change', async () => {
    const s = await setup({"validate": "change"});
    expect(s.el.shadowRoot ? s.root.childElementCount > 0 : s.el.isConnected).toBe(true);
  });
});
