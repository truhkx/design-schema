...
- `ArrowUp` (Moves to the previous tab, wrapping; selects it under automatic activation.): expect focus-prev; given `orientation: "vertical"`; story URL `/iframe.html?id=tabs-react--keyboard&viewMode=story&args=orientation:vertical`
...
notes: "<div role=\"tablist\" aria-label> of <button role=\"tab\" aria-selected aria-controls\
  \ tabindex={0|-1}>; panels are <div role=\"tabpanel\" aria-labelledby tabindex=\"\
  0\"> (focusable so Tab from the list lands on the panel content region). `aria-controls`\
  \ is set only while the tab's panel is mounted (with `keepMounted: false` an unselected\
  \ panel is not in the DOM, so there is no id to point at). The React panel's DOM\
  \ id is its `id` prefixed with a `useId` base (and tab ids use the same base), so\
  \ two Tabs on a page cannot collide; the `id` prop is only the match key. The indicator\
  \ is a pseudo-element or an absolutely positioned bar animated between tabs. The\
  \ tab list scrolls along its orientation with overflow when tabs exceed its size,\
  ...
  \ - tab.offsetLeft - tab.offsetWidth` plus the list's scroll offset), not taken\
  \ from `offsetLeft` directly. Keeping the selected tab in view is measured the same\
  \ way, from the tab's offset within the list's client box rather than from an assumed\
  \ non-negative `scrollLeft`, so it works in both directions (RTL browsers report\
  \ a negative `scrollLeft`). Every prop a keyboard rule names in its `given` needs\
  \ an explicit enum `argType` in the Keyboard story's meta (`activation`, `orientation`,\
  \ `fit`): react-docgen infers none for these props, and Storybook silently drops\
  \ a URL arg it cannot map \u2014 the gate then tests the default orientation and\
  \ blames the keyboard model."
```
