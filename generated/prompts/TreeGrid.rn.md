...
notes: 'DataGrid''s list over the flattened visible rows (role="grid" via the ARIA-aligned
  ...
  }} when it has children, accessibilityLabel of rowName, copy.level and copy.childCount
  joined with ", " (the count is left out for leaves and for lazy rows not yet loaded),
  and its own accessibilityActions expand/collapse. Native has no row position in
  set: only level and child count are conveyed. The expand Button is a real touch
  target (minimum size) since there are no arrow keys, but it is hidden from accessibility
  (importantForAccessibility no-hide-descendants / accessibilityElementsHidden): screen
  readers reach expansion through the row header''s actions. Pressing a row header
  with children toggles it; long-pressing an editable row header edits. The keyboard
  table has no native path beyond Enter and the editor''s Enter/Escape: expand/collapse
  actions, the root expandAll/collapseAll (every loaded row, not only siblings), and
  the select Checkboxes stand in; `*`, Shift+Space and Ctrl+A have no native equivalent.'
```
