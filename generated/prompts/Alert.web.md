...
notes: "role=\"status\" | \"alert\" from `live` (each implies its aria-live; set only\
  ...
  \ then the name, which is intended (a status region is named by its content). On\
  \ dismiss, \"next focusable\" means a, button, input, select, textarea, [tabindex]\
  \ \u2265 0 or contenteditable in document order after the alert, skipping disabled\
  \ elements, elements inside `inert`, and elements that are not rendered (hidden\
  \ or display none)."
```
