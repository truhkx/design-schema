...
notes: 'Uses the native Switch for platform-native feel; trackOn/trackOff map to trackColor
  ...
  are not applied); a disabled native Switch is not focusable. The label row is a
  Pressable wrapping the Switch so the whole row toggles. The native Switch draws
  its own track and thumb, so trackWidth/trackHeight/thumbSize/thumbInset/radius are
  not overridable on native; the gap and text bindings are.'
```
