...
export function Stack({
  ...
  const classes = [
    ...
    `demo-stack--gap-${gap}`,
    `demo-stack--align-${align}`,
    `demo-stack--justify-${justify}`,
    wrap ? 'demo-stack--wrap' : null,
    isList ? 'demo-stack--list' : null,
    className ?? null,
  ]
    .filter(Boolean)
    .join(' ');
      ...
      {isList
        ? Children.map(children, (child) =>
            child === null || child === undefined || typeof child === 'boolean' ? null : (
              <li role="listitem" data-part="item" className="demo-stack__item">
                {child}
              </li>
            ),
          )
