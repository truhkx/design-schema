...
notes: 'A native Modal sheet: a View with accessibilityRole="menu" containing Pressable
  ...
  rows are `role="menuitem"`. `accessibilityViewIsModal` is a View prop set on the
  surface View; the Modal itself takes visible, transparent and onRequestClose. The
  heading Text gets tone="muted" and size="sm" only: `element: p` is web and Lit only,
  since rn Text has no element prop. Pressable has no key events, so there are no
  arrow keys, no Home/End and no roving tabindex; each row is its own accessibility
