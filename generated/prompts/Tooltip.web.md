...
notes: "The child is cloned with aria-describedby (or aria-labelledby) pointing at\
  ...
  \ The visually-hidden span is rendered as a sibling immediately after the trigger\
  \ \u2014 it is not focusable and carries no item role, so a parent composite (Toolbar's\
  \ roving tabindex, its overflow measurement) that enumerates element children sees\
  \ it but never treats it as an item. `children` is typed as a single `ReactElement`,\
  \ since it is cloned."
```
