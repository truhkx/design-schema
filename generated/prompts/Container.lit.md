...
notes: 'The host is the column (`:host { display: block }`) with a default slot. Same
  ...
  section is a region only when it is named. The role stays on ElementInternals, as
  in Box, with no `role` attribute added to the host; the main-landmark scenario is
  web-only because the test accessibility lookup cannot read ElementInternals. The
  host carries `data-part="column"`. `element` is an attribute-settable property that
  ...
  defaults (the content max-width and the responsive default gutter).'
```
