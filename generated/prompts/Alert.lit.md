...
- live
- dismissible
notes: "The role is set on the host element via ElementInternals so the live region\
  \ is in the light DOM tree where assistive technology expects it. `dismiss` is a\
  \ composed CustomEvent; the inner button's `press` is stopped so consumers see one\
  \ event. `heading` is a property (attribute `heading`) or the named slot `heading`;\
  \ body is the default slot. Accessible name: the host is named by aria-labelledby\
  \ the heading when present, else by the body text (a status region is named by its\
  \ content), via ElementInternals ariaLabelledByElements where supported and aria-label\
  \ with the text otherwise \u2014 ids never cross the shadow root, so a literal aria-label\
  \ is the fallback, not an idref. Shadow parts carry the anatomy names in kebab-case\
  \ (`dismiss-button`, not `dismiss`)."
```
