...
notes: "A View with height (or width) = border.width.thin and backgroundColor color.border,\
  ...
  \ so the author knows the boundary is silent on this platform; that warning is keyed\
  \ on `semantic` and whether a label is in effect, so it also fires for a semantic\
  \ vertical divider whose label is ignored. The composition's `element: span` is\
