...
component:
  ...
  props:
    ...
    color:
      ...
      platforms: [rn]
  styles:
    size: { token: 'font.size.{size}', description: 'The glyph box is a square of 1em; `size` sets that em (font-size on the element), so the box tracks the type scale. With `inline`, font-size is inherited instead. Non-inline icons are display inline-block with vertical-align middle; inline ones sit at vertical-align -0.125em.' }
    color: { token: color.foreground, description: 'Only when the icon has no colored ancestor: glyphs are drawn in currentColor and inherit the text color, so a Button, Link or Alert colors them for free. On React Native, where there is no currentColor, the parent passes its foreground token explicitly and this is the fallback.' }
    strokeWidth: { token: border.width.focus, description: 'Stroke thickness of line glyphs (check, dash, chevrons, close, plus, minus, external, search, arrows, calendar), in screen pixels at every size (vector-effect non-scaling-stroke), so glyphs stay legible at xs. Filled glyphs (the four status shapes, ellipsis) have no stroke: each is one evenodd path whose inner mark is a hole. Web and Lit only; a Text glyph on React Native has no stroke.' }
  a11y:
    role: img
  ...
  platforms:
    ...
    rn:
      element: Text
      props: [accessibilityRole=image, accessibilityLabel, accessibilityElementsHidden, importantForAccessibility]
      notes: 'No SVG in core React Native and no new dependencies, so glyphs are rendered as Text from a Unicode table — check ✓, dash –, chevrons › ⌄ ⌃ ‹, close ×, plus +, minus −, info ⓘ, success ✔, warning ▲, danger ⊗, external ↗, ellipsis …, search ⌕, arrows → ←, calendar ▦ — in a square box (fontSize, lineHeight, width and height all the size token; textAlign center; includeFontPadding false), colored by the `color` prop (no currentColor), allowFontScaling left on so icons scale with the text beside them. Known risks: ⌕, ⊗ and the ⌄/⌃ chevrons are not guaranteed in every Android system font and may render as tofu; the four status shapes are only approximately distinct as characters. This is the acknowledged platform limit; adopting react-native-svg is the one dependency decision that lifts it, and the API is designed so that swap changes nothing for consumers.'
---
...
### React Native
Render `Text` with the glyph from a `GLYPHS` table keyed by `name`. Sizing: `fontSize`, `lineHeight`, `width` and `height` all from `font.size.{size}` for a square box. With `inline` nested in the system Text (`TextNestingContext` is true), set no fontSize, fontFamily or color so RN's nested-Text inheritance applies; with `inline` outside a Text (inside a Button's View, say) there is nothing to inherit, so use `font.size.md`. `color` comes from the prop, falling back to `color.foreground` only when not inheriting. Decorative: `accessibilityElementsHidden` and `importantForAccessibility="no"`; labelled: `accessibilityRole="image"`, `accessibilityLabel`, `importantForAccessibility="auto"`. `strokeWidth` does not apply to a Text glyph. Glyph choices must exist in the default iOS and Android system fonts; the table above does. Document in the JSDoc that this is a Unicode fallback and that swapping to react-native-svg is a drop-in change.
