...
async function setup(given: Record<string, unknown> = {}) {
  ...
  const s = {
    ...
    props,
    root_: () => el,
    list: () => ((el.matches('[role="listbox"]') ? el : null) ?? deep(root, '[role="listbox"]') ?? (el.matches('[part~="list"], [data-part="list"]') ? el : null) ?? deep(root, '[part="list"]') ?? deep(root, '[data-part="list"]') ?? root.firstElementChild) as HTMLElement,
    option: () => ((el.matches('[part~="option"], [data-part="option"]') ? el : null) ?? deep(root, '[part="option"]') ?? deep(root, '[data-part="option"]')) as HTMLElement,
  };
  return s;
}
...
describe('ds-listbox', () => {
  test('click-on-an-option-selects-it', async () => {
    const s = await setup({});
    await userEvent.click(located(activatable(s.option(), s.el)), { force: true });
  ...
  test('has-accessible-name', async () => {
    ...
    else expect(s.list()).toHaveAccessibleName(s.props.label);
  });
