...
test.describe('Listbox (web) keyboard', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/iframe.html?id=listbox-react--keyboard&viewMode=story');
  ...
  test('ArrowDown: Moves to the next enabled option (and selects it when selection follows focus).', async ({ page }) => {
    const root = await subject(page, page.getByRole('listbox').first());
  ...
  test.skip('typeahead letter: Typeahead to the next option whose label starts with the typed characters; typing the same character repeatedly cycles through the options starting with it (APG) — a buffer of one letter repeated collapses back to that single letter and searches from the option after the active one, while any other buffer is a prefix searched from the active one. The buffer clears after `typeaheadReset`. — manual', async () => {});
  test.skip('PageDown: Moves by the `maxVisible` count of enabled options — not of drawn rows, so disabled rows and group labels do not consume the move — clamped to the last/first enabled option; with `maxVisible: all` jumps to the last/first enabled option. Follows `selectionFollowsFocus` like the arrows. — manual', async () => {});
  test.skip('PageUp: Moves by the `maxVisible` count of enabled options — not of drawn rows, so disabled rows and group labels do not consume the move — clamped to the last/first enabled option; with `maxVisible: all` jumps to the last/first enabled option. Follows `selectionFollowsFocus` like the arrows. — manual', async () => {});
});
