...
async function setup(given: Record<string, unknown> = {}) {
  ...
  const s = {
    ...
    props,
    root_: () => el,
    scrim: () => ((el.matches('[role="menu"]') ? el : null) ?? deep(root, '[role="menu"]') ?? (el.matches('[part~="scrim"], [data-part="scrim"]') ? el : null) ?? deep(root, '[part="scrim"]') ?? deep(root, '[data-part="scrim"]') ?? root.firstElementChild) as HTMLElement,
...
describe('ds-action-sheet', () => {
  ...
  test('the-cancel-row-fires-on-close', async () => {
    ...
    await userEvent.click(located(activatable(s.cancelButton(), s.el)), { force: true });
