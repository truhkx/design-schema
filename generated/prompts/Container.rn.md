...
notes: "View with maxWidth, alignSelf (center \u2192 center, start \u2192 flex-start),\
  ...
  \ ref. The root is the `column` part and keeps `testID=\"Container\"` with no separate\
  \ part testID, as Box does for `surface`. On phones the cap rarely applies; on tablets\
  \ and react-native-web it does. The gutter reads the window width, never the parent's,\
  \ so a nested `gutter: default` Container picks its gutter by the window here while\
