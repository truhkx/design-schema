...
## 2026-09-23 13:39 — round 1

- Text: packages/rn/src/Text.stories.tsx was reverted on disk mid-run by an outside process, undoing the two spec-required story strings (Default children 'Use the email you signed up with.', ToneOnAction children 'Text on an action background'); they were not reapplied and still need fixing.
- Text: the enum stories other than ToneDanger/ToneOnAction are told to use the Default children, but the examples section also gives BodyCopy its own children; I kept both (Default = 'Use the email you signed up with.', BodyCopy = the example's text).
- Text: the `inline-error-wording` example is web/lit-only (it sets `element: span`), yet the tone description requires ToneDanger to use its children 'on every platform'; on RN I used the children only, without `element`, and made no InlineErrorWording story.
