...
notes: The <form> lives in the shadow root; when `action` is set, submit navigates
  ...
  `reportValidity()` is also always true, and formResetCallback restores `defaultValue`
  without firing `change` or `clear`. The label is a `<label for>` wrapping `<ds-text
  element="span">`; data-part="label" goes on the ds-text and the visually-hidden
  class on the `<label>`. Inside-a-Form detection walks up the composed tree (crossing
  shadow roots through getRootNode().host) for a ds-form, not closest() alone.
```
