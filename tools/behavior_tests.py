...
platform the scenario applies to (`behavior_for` below narrows authored + derived scenarios per
platform; it mirrors `behaviorFor` in tools/parse.ts until this tool is ported too):
...
# ---- mirrored from tools/parse.ts (accessibleNameProp, mergedBehavior, behaviorFor) ----
# The parser is TypeScript now (process/typescript-and-currency.md, Workstream A); this tool is ported in a later
# step and until then carries the two small pieces of parser logic it needs. Keep them identical to parse.ts.
...
def behavior_for(component: dict, derived: list[dict], platform: str) -> list[dict]:
    ...
    return result
# ---- end of the mirrored parser logic ----
...
def then_name_lines(c: dict, platform: str) -> list[str]:
    ...
    name_prop = accessible_name_prop(c)
    ...
    for entry in entries:
        ...
        for platform in PLATFORMS:
            ...
            if not (ROOT / SOURCE_FILE[platform].format(name=c["name"])).exists():
                ...
                continue
            scenarios = behavior_for(c, derived, platform)
            if not scenarios:
                continue
