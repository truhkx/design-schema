...
async function setup(given: Record<string, unknown> = {}) {
  ...
  const s = {
    ...
    props,
    root_: () => el,
    label: () => ((el.matches('[role="spinbutton"]') ? el : null) ?? deep(root, '[role="spinbutton"]') ?? (el.matches('[part~="label"], [data-part="label"]') ? el : null) ?? deep(root, '[part="label"]') ?? deep(root, '[data-part="label"]') ?? root.firstElementChild) as HTMLElement,
    decrementButton: () => ((el.matches('[part~="decrementButton"], [data-part="decrementButton"]') ? el : null) ?? deep(root, '[part="decrementButton"]') ?? deep(root, '[data-part="decrementButton"]')) as HTMLElement,
    incrementButton: () => ((el.matches('[part~="incrementButton"], [data-part="incrementButton"]') ? el : null) ?? deep(root, '[part="incrementButton"]') ?? deep(root, '[data-part="incrementButton"]')) as HTMLElement,
  };
  return s;
}
...
describe('ds-number-input', () => {
  test('the-increment-button-steps-up', async () => {
    ...
    await userEvent.click(...(await pointerAt(activatable(s.incrementButton(), s.el), s.incrementButton())));
    expect(s.events.onChange).toHaveBeenCalled();
  });
  test('the-decrement-button-steps-down', async () => {
    const s = await setup({"defaultValue": 5, "step": 1});
    await userEvent.click(...(await pointerAt(activatable(s.decrementButton(), s.el), s.decrementButton())));
    expect(s.events.onChange).toHaveBeenCalled();
  });
  ...
  test('a-disabled-field-does-not-step', async () => {
    ...
    await userEvent.click(located(activatable(s.incrementButton(), s.el)), { force: true });
    expect(s.events.onChange).not.toHaveBeenCalled();
  });
  test('renders', async () => {
    const s = await setup({});
