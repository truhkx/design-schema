...
notes: Custom thumbs (<div role="slider" tabindex="0">) on a track rather than <input
  ...
  Texts sit in wrappers the Slider owns, which carry their data-part; the errorMessage
  wrapper is role=alert, as on Lit. `copy.pageUpAction`, `pageDownAction`, `homeAction`
  and `endAction` are React Native action labels; web and Lit do not render them.
```
