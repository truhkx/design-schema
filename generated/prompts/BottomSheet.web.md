...
- aria-labelledby
notes: "The same native <dialog> as Dialog, filling the viewport, with the surface\
  \ at the bottom edge (inset-block-end: 0), `max-inline-size` from the maxWidth token\
  \ and `margin-inline: auto`, so it is full width on a phone and capped and centred\
  \ on a wide screen \u2014 one presentation at every width, with no media query and\
  \ no Dialog. Drag uses Pointer Events on the handle/header with setPointerCapture;\
  \ the handle is aria-hidden and not focusable. Safe-area padding via env(safe-area-inset-bottom).\
  \ The <dialog> fills the viewport with a transparent ::backdrop and the scrim is\
  \ a real `data-part=\"scrim\"` element inside it, as in Dialog; a scrim dismiss\
  \ is a `click` whose target is that element. Button, Heading and Stack write their\
  \ own data-part, so the heading, closeButton and footer parts are sheet-owned wrappers\
  \ around them; the body Box takes its data-part directly, inside the sheet-owned\
  \ scroll element that `partGap` describes. FocusScope writes its own `data-part=\"\
  scope\"`, so the `focusScope` part is a sheet-owned element directly inside FocusScope,\
  \ as in Dialog. `container` is the portal target every portaled overlay accepts\
  \ (not a schema prop). `ref` resolves to the sheet's <dialog>, null while closed."
```
