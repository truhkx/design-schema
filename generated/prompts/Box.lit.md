...
notes: "The host is the box (`:host { display: block }`) with a default slot, so children\
  ...
  \ and the element cannot see where it sits; a page-level banner is Landmark. Box\
  \ removes only a role it wrote itself, so a consumer `role` on a `div` box stays.\
  \ A derived `renders` scenario on Lit means the host is connected and its shadow\
  \ root holds the slot."
```
