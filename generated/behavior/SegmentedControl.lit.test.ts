...
async function setup(given: Record<string, unknown> = {}) {
  ...
  const s = {
    ...
    segment: () => ((el.matches('[part~="segment"], [data-part="segment"]') ? el : null) ?? deep(root, '[part="segment"]') ?? deep(root, '[data-part="segment"]')) as HTMLElement,
  };
  return s;
}
...
describe('ds-segmented-control', () => {
  test('click-selects-a-segment', async () => {
    ...
    await userEvent.click(located(activatable(s.segment(), s.el)), { force: true });
  ...
  test('has-accessible-name', async () => {
    ...
    else expect(s.group()).toHaveAccessibleName(s.props.label);
  });
});
