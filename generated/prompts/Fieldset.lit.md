...
notes: Shadow root with a <fieldset><legend> and a default slot for the fields, which
  ...
  web. The legend and description render their text through <ds-text> inside the native
  <legend>/<p>, so legendSize, legendWeight and helperSize reach it as Text overrides
  rather than as Fieldset's own rules. `disabled` is propagated to slotted ds-* fields
