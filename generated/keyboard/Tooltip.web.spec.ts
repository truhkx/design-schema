// Generated by tools/keyboard_tests.ts from the `keyboard` block of the Tooltip doc. Do not edit.
...
test.describe('Tooltip (web) keyboard', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/iframe.html?id=tooltip-react--keyboard&viewMode=story');
    await expect(page.getByRole('tooltip').first()).toBeVisible({ timeout: 15_000 });
  });
  test('Escape: Hides the tooltip without moving focus. (tooltip visible)', async ({ page }) => {
    const root = await subject(page, page.getByRole('tooltip').first());
    
    const before = await focusIndex(page, root);
    const stateBefore = await ariaState(page);
    void before; void stateBefore;
    const held = await holdFocus(page);
    await page.keyboard.press('Escape');
    await expect(page.locator('[data-part="popup"]').first()).toBeHidden();
    expect(held, 'nothing to hold focus').toBe(true); expect(await focusHeld(page)).toBe(true);
  });
});
