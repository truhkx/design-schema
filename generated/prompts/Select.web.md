...
notes: "The APG select-only combobox: the trigger is a <button role=\"combobox\" aria-haspopup=\"\
  ...
  \ Select, which toggles the active option. Listbox applies initialActiveValue only\
  \ when it receives focus, and focus never leaves the trigger, so on open Select\
  \ dispatches a `focusin` on the Listbox ref to make the selected (or first) option\
  \ active. A hidden <input name> per selected value carries the value(s) for native\
  \ form submission \u2014 none when nothing is selected or when disabled. `native:\
  \ always` renders <select> (and <select multiple>) with the same label/description/error\
