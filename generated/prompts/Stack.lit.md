...
notes: 'The host is the flex container (`:host { display: flex }`) with a default
  ...
  them, which would re-fire slotchange forever. `element` is not reflected: it is
  read from the attribute or property but is not a styling contract, since it changes
  only the shadow structure.'
```
