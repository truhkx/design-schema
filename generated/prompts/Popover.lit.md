...
notes: "Slots: `trigger` and default. The panel renders in the shadow root with the\
  ...
  \ aria-hidden and tabindex=-1 elements; a positive tabindex is not reordered). Shift+Tab\
  \ from the first element prevents default and focuses the trigger; in both directions\
  ...
  \ and the heading wrapper draws the ring with `:has(:focus-visible)`. A panel `data-side`\
  \ holds the resolved physical side, as on web. Composed `open-change`."
```
