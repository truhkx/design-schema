...
notes: "`items` as a property (`content` and `actions` typed as any lit-html renderable);\
  ...
  \ role=\"status\" new-items row and aria-busy on the host follow the web notes.\
  \ Ctrl+Home/End walk the document for focusables (shadow-piercing, as FocusScope).\
  \ No `shadowRootOptions.delegatesFocus`: the host is not focusable and delegating\
  \ would pull focus into the first article on any click in the feed's whitespace\
  \ \u2014 the package declares it for focusable hosts only. After `show-new` the\
  \ first new article is scrolled into view (smooth normally, instant under `prefers-reduced-motion`),\
  \ which is the feed's only motion on this platform. Composed `load-more`, `show-new`,\
  \ `item-visible`."
```
