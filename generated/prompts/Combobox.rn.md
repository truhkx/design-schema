...
notes: "On phones the popup is a BottomSheet with the TextInput at its top (keyboard-avoiding)\
  ...
  \ render before the input inside the field; each chip has a remove Button. Result\
  \ counts are announced with announceForAccessibility. Form registration as Input."
```
