...
- accessibilityViewIsModal
notes: "There is no Tab order to confine on native. trapped maps to accessibilityViewIsModal\
  \ on the wrapper View (VoiceOver/TalkBack ignore siblings). There is no way to walk\
  \ arbitrary children for a focusable descendant, so `first`, `last` and `container`\
  \ all call AccessibilityInfo.setAccessibilityFocus on the wrapper View and only\
  \ `none` differs \u2014 the screen reader then reads the scope from its top, which\
  \ is the intended result for all three. restoreFocus can only capture an opener\
  \ that is a TextInput (TextInput.State.currentlyFocusedInput is the one \"what is\
  \ focused\" native exposes), which is why `returnFocusTo` is required here for every\
  \ other kind of trigger. onEscapeAttempt never fires on this platform: nothing can\
  \ attempt to leave a Tab order that does not exist. Hardware-keyboard Tab wrapping\
