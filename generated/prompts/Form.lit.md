  ...
  props:
    ...
    errorSummary:
      ...
      description: 'When submission fails validation, render a summary of errors above
        the fields that links to each field. Each item reads "Label: message"; every
        field''s registration carries its `label` for exactly this, on all three platforms.'
  ...
  platforms:
    ...
    lit:
      ...
      notes: Wraps a native <form novalidate> in the shadow root, but form ownership
        ...
        needs no change here. It submits on a composed `press` from a ds-button[type=submit]
        and on Enter in a ds-input, and propagates `disabled` to the fields it found
        (remembering which it disabled). A field missing an `id` is given `{name}-{field.name}`
        at submit time so the error summary can link to it. Dispatches `submit` with
        `{ values }` and `invalid` with `{ errors }` in detail. Never nest inside
        a native form.
    rn:
      element: View
      props:
...
- `onSubmit`: emit `submit`
  - payload, the keys of `CustomEvent.detail`: `values: Record<string, string | boolean>`
  - fires on: user
...
notes: "Wraps a native <form novalidate> in the shadow root, but form ownership is\
  ...
  \ here. It submits on a composed `press` from a ds-button[type=submit] and on Enter\
  \ in a ds-input, and propagates `disabled` to the fields it found (remembering which\
  \ it disabled). A field missing an `id` is given `{name}-{field.name}` at submit\
  \ time so the error summary can link to it. Dispatches `submit` with `{ values }`\
  \ and `invalid` with `{ errors }` in detail. Never nest inside a native form."
```
