...
export const Icon = function Icon({
  ...
  // `.ds-icon--inline` (later in the cascade) takes the font size from the surrounding text instead.
  const classes = ['ds-icon', `ds-icon--${size}`, inline ? 'ds-icon--inline' : null, className ?? null]
    .filter(Boolean)
    .join(' ');
