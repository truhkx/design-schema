// Generated by tools/keyboard_tests.ts from the `keyboard` block of the Accordion doc. Do not edit.
...
test.describe('Accordion (web) keyboard', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/iframe.html?id=accordion-react--keyboard&viewMode=story');
    await expect(page.locator('[data-ds="Accordion"]').first()).toBeVisible({ timeout: 15_000 });
  });
  test('Enter: Toggles the focused section.', async ({ page }) => {
    const root = await subject(page, page.locator('[data-ds="Accordion"]').first());
  ...
  test(' : Toggles the focused section.', async ({ page }) => {
    const root = await subject(page, page.locator('[data-ds="Accordion"]').first());
    await focusAt(page, root, 0);
    const before = await focusIndex(page, root);
    const stateBefore = await ariaState(page);
  ...
  test('ArrowDown: Moves focus to the next trigger; wraps.', async ({ page }) => {
    const root = await subject(page, page.locator('[data-ds="Accordion"]').first());
    await focusAt(page, root, 0);
    const before = await focusIndex(page, root);
    const stateBefore = await ariaState(page);
  ...
  test('ArrowUp: Moves focus to the previous trigger; wraps.', async ({ page }) => {
    const root = await subject(page, page.locator('[data-ds="Accordion"]').first());
  ...
  test('ArrowDown: From the last trigger wraps to the first.', async ({ page }) => {
    const root = await subject(page, page.locator('[data-ds="Accordion"]').first());
    await focusAt(page, root, await focusableCount(page, root) - 1);
    const before = await focusIndex(page, root);
    const stateBefore = await ariaState(page);
  ...
  test('Home: First trigger.', async ({ page }) => {
    const root = await subject(page, page.locator('[data-ds="Accordion"]').first());
    await focusAt(page, root, await focusableCount(page, root) - 1);
    const before = await focusIndex(page, root);
    const stateBefore = await ariaState(page);
  ...
  test('End: Last trigger.', async ({ page }) => {
    const root = await subject(page, page.locator('[data-ds="Accordion"]').first());
    await focusAt(page, root, 0);
    const before = await focusIndex(page, root);
    const stateBefore = await ariaState(page);
  ...
  test('Tab: Ordinary tab order — every trigger is a tab stop (the APG recommends this so panel content stays reachable).', async ({ page }) => {
    const root = await subject(page, page.locator('[data-ds="Accordion"]').first());
    await focusAt(page, root, 0);
    const before = await focusIndex(page, root);
    const stateBefore = await ariaState(page);
