...
notes: '`items` is a property (.items=${[...]}). The nav and list are rendered in
  ...
  with no part. `copy.current` is not rendered. The landmark role is asserted on web
  only; Lit tests cover the nav through the derived accessible-name scenario and do
  not assert role=navigation separately.'
```
