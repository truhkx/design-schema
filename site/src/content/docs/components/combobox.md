...
component:
  ...
  props:
    ...
    clearable:
      type: boolean
      default: true
      description: 'Show a clear button when there is a value or text. Lit attribute: `no-clear`.'
  events:
    onChange:
  ...
  keyboard:
    ...
    - { keys: [Alt+ArrowDown], action: 'Opens the list with the selected option active, or no active option when nothing is selected; does nothing while the list is already open.', from: first, expect: manual }
  styles:
    ...
    iconColor: { token: color.foreground.muted, description: 'Toggle chevron, clear and chip-remove icons; forwarded to each composed Icon''s own `color` override as this token. It has no --ds-combobox-* hook, since the hook could not reach the child without restyling it.' }
