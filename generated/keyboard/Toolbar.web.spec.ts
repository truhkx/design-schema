// Generated by tools/keyboard_tests.ts from the `keyboard` block of the Toolbar doc. Do not edit.
...
test.describe('Toolbar (web) keyboard', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/iframe.html?id=toolbar-react--keyboard&viewMode=story');
  ...
  test('ArrowRight: Next control (ArrowDown when vertical). Skips disabled controls; does not wrap.', async ({ page }) => {
    const root = await subject(page, page.getByRole('toolbar').first());
  ...
  test('ArrowLeft: Previous control (ArrowUp when vertical).', async ({ page }) => {
    const root = await subject(page, page.getByRole('toolbar').first());
  ...
  test('Home: First control.', async ({ page }) => {
    const root = await subject(page, page.getByRole('toolbar').first());
    await focusAt(page, root, await focusableCount(page, root) - 1);
    const before = await focusIndex(page, root);
    const stateBefore = await ariaState(page);
  ...
  test('End: Last control.', async ({ page }) => {
    const root = await subject(page, page.getByRole('toolbar').first());
    await focusAt(page, root, 0);
    const before = await focusIndex(page, root);
    const stateBefore = await ariaState(page);
