...
notes: 'A View row of Pressables with accessibilityRole="radio" and accessibilityState={{
  checked, disabled }}; the pill is an Animated.View hidden from assistive technology
  (accessibilityElementsHidden, importantForAccessibility="no-hide-descendants"),
  as Tabs hides its indicator. Not registered with FormContext. Each segment is its
  ...
  roving `focusable`), and the arrow scenarios are web and Lit only. No Tooltip part
  ...
  (that would merge the segments into one stop), so tests find it by testID and assert
  role and name rather than getByRole. iOS''s UISegmentedControl look is approximated
