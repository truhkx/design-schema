...
}`,...x.parameters?.docs?.source},description:{story:`For the keyboard gate: the closed field with a query and suggestions, so the input, the clear\r
button (there is text) and the submit button are the three focus stops the Tab rule walks, and the\r
first ArrowDown opens the list. There is no \`open\` prop — focus alone never opens it.`,...x.parameters?.docs?.description}}}})))()}C();export{l as Default,_ as Disabled,h as FilterWithinAResultsPage,f as HeaderSearch,x as Keyboard,v as Loading,y as NoSuggestions,p as SearchPageHero,d as SizeLg,u as SizeMd,b as SuggestionsFromOnChange,m as WithSuggestions,g as WithValue,S as __namedExportsOrder,c as default};