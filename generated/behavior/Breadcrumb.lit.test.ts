...
async function setup(given: Record<string, unknown> = {}) {
  ...
  const s = {
    ...
    props,
    root_: () => el,
    nav: () => ((el.matches('[role="navigation"]') ? el : null) ?? deep(root, '[role="navigation"]') ?? (el.matches('[part~="nav"], [data-part="nav"]') ? el : null) ?? deep(root, '[part="nav"]') ?? deep(root, '[data-part="nav"]') ?? root.firstElementChild) as HTMLElement,
    link: () => ((el.matches('[part~="link"], [data-part="link"]') ? el : null) ?? deep(root, '[part="link"]') ?? deep(root, '[data-part="link"]')) as HTMLElement,
    current: () => ((el.matches('[part~="current"], [data-part="current"]') ? el : null) ?? deep(root, '[part="current"]') ?? deep(root, '[data-part="current"]')) as HTMLElement,
  };
  return s;
}
...
describe('ds-breadcrumb', () => {
  test('click-on-an-ancestor-reports-navigation', async () => {
    const s = await setup({});
    await userEvent.click(...(await pointerAt(activatable(s.link(), s.el), s.link())));
  ...
  test('an-uncollapsed-trail-shows-every-ancestor', async () => {
    ...
    expect(flatText(s.el)).toMatch(new RegExp("Components"));
    expect(flatText(s.el)).toMatch(new RegExp("Navigation"));
  });
  test('renders', async () => {
    const s = await setup({});
  ...
  test('has-accessible-name', async () => {
    ...
    else expect(s.nav()).toHaveAccessibleName(s.props.label);
  });
});
