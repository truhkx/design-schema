...
async function setup(given: Record<string, unknown> = {}) {
  ...
  const s = {
    ...
    props,
    root_: () => el,
    region: () => ((el.matches('[role="region"]') ? el : null) ?? deep(root, '[role="region"]') ?? (el.matches('[part~="region"], [data-part="region"]') ? el : null) ?? deep(root, '[part="region"]') ?? deep(root, '[data-part="region"]') ?? root.firstElementChild) as HTMLElement,
    prevButton: () => ((el.matches('[part~="prevButton"], [data-part="prevButton"]') ? el : null) ?? deep(root, '[part="prevButton"]') ?? deep(root, '[data-part="prevButton"]')) as HTMLElement,
    nextButton: () => ((el.matches('[part~="nextButton"], [data-part="nextButton"]') ? el : null) ?? deep(root, '[part="nextButton"]') ?? deep(root, '[data-part="nextButton"]')) as HTMLElement,
    pickerItem: () => ((el.matches('[part~="pickerItem"], [data-part="pickerItem"]') ? el : null) ?? deep(root, '[part="pickerItem"]') ?? deep(root, '[data-part="pickerItem"]')) as HTMLElement,
  };
  return s;
}
...
describe('ds-carousel', () => {
  test('next-advances-a-slide', async () => {
    const s = await setup({});
    await userEvent.click(...(await pointerAt(activatable(s.nextButton(), s.el), s.nextButton())));
    expect(s.events.onChange).toHaveBeenCalled();
  });
  test('previous-at-the-first-slide-does-nothing', async () => {
    const s = await setup({});
    await userEvent.click(...(await pointerAt(activatable(s.prevButton(), s.el), s.prevButton())));
  ...
  test('the-picker-jumps-straight-to-a-slide', async () => {
    ...
    await userEvent.click(...(await pointerAt(activatable(s.pickerItem(), s.el), s.pickerItem())));
    expect(s.events.onChange).toHaveBeenCalled();
  });
  test('renders', async () => {
    const s = await setup({});
  ...
  test('has-accessible-name', async () => {
    ...
    else expect(s.region()).toHaveAccessibleName(s.props.label);
  });
});
