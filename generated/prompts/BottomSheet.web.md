...
notes: 'The same native <dialog> as Dialog, positioned at the bottom edge with inset-block-end:
  ...
  via env(safe-area-inset-bottom). Button, Heading and Stack write their own data-part,
  so the closeButton part is the sheet-owned wrapper around the Button. In the wide
  presentation the root is Dialog''s own <dialog> (its hooks, not a BottomSheet wrapper),
  and `ref` resolves to that <dialog>; below the breakpoint `ref` resolves to the
  sheet''s <dialog>, null while closed.'
```
