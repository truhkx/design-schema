...
notes: "A <div role=\"feed\" aria-label aria-busy> of Cards (`focusable`, so each\
  ...
  \ a Feed-owned wrapper inside the items column, like the end message, and is not\
  ...
  \ goes on the element whose direct children are the articles, not on an ancestor:\
  \ ARIA requires a feed to own its article children, and axe reports aria-required-children\
  \ when a wrapper sits between them. So the items column itself carries role=\"feed\"\
  \ (with the label and aria-busy), the new-items role=\"status\" row and any other\
  \ chrome stay outside it as siblings, and each Feed-owned article wrapper carries\
  \ role=\"article\" with aria-posinset/aria-setsize/aria-describedby, while the Card\
  \ inside it is the presentation. Put another way: nothing may sit between the feed\
  \ element and an article element."
```
