...
notes: "There is no Tab order to confine on native. `trapped && active` maps to accessibilityViewIsModal\
  ...
  \ AccessibilityInfo.setAccessibilityFocus on the wrapper View and only `none` differs\
  \ \u2014 the screen reader then reads the scope from its top, which is the intended\
  \ result for all three. restoreFocus can only capture an opener that is a TextInput\
  \ (TextInput.State.currentlyFocusedInput is the one \"what is focused\" native exposes),\
  \ which is why `returnFocusTo` is required here for every other kind of trigger.\
  \ onEscapeAttempt never fires on this platform: nothing can attempt to leave a Tab\
  \ order that does not exist. Hardware-keyboard Tab wrapping is not implemented;\
  ...
  \ instead. Neither `role` nor `accessibilityRole` nor `accessibilityLabel` is set\
  \ on the wrapper."
```
