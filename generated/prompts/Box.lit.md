...
notes: "The host is the box (`:host { display: block }`) with a default slot, so children\
  \ stay in the light DOM. `element` swaps nothing in the shadow root \u2014 the host\
  \ is the element, so `element` is accepted for API parity and sets `role` via ElementInternals\
  \ only for sectioning values (article, aside, header, footer, main, nav map to their\
  \ implicit roles; div and section set none)."
```
