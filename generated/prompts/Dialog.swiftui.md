...
notes: "Presented with `.sheet` on compact width and `.popover` (regular width, iPad);\
  \ the `size` enum is sm, md and lg only \u2014 there is no `full` value \u2014 and\
  \ `.fullScreenCover` is used for `lg` on compact width, where a sheet would otherwise\
  \ leave a sliver of the page. The dialog surface, heading (`Heading`, the `.accessibilityLabel`\
  \ of the container), body and actions are the package's own views inside the presentation\
  \ with `.presentationBackground(color.overlay.surface)` and `.presentationDragIndicator(.hidden)`.\
