...
notes: "No `element` prop \u2014 RN has one Text primitive. `truncate` maps to `numberOfLines={1}`.\
  ...
  \ this object. The ref is `Ref<TextInstance>`, the root's own instance type. Outside\
  \ any Text the context reads `{ fontSize: 0, color: '', nested: false }`, so a consumer\
  \ branches on `nested` and never on the numbers. `a11y.role: generic` has no native\
