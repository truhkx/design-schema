...
notes: "Tablets and react-native-web only; below the stackBelow width the panes always\
  \ stack and the separator is not rendered. The separator is a plain View with a\
  \ PanResponder (as Slider's thumb \u2014 panHandlers on a Pressable fight its own\
  \ responder), so it cannot show a keyboard focus ring itself (a known platform limit;\
  \ the composed collapse Button has full focus treatment). accessibilityRole=\"adjustable\"\
  , accessibilityValue={{ min, max, now, text }}, accessibilityActions increment/decrement\
  \ (step; the system names them), setMinimum/setMaximum (Home/End; labelled copy.setMinimum/copy.setMaximum)\
  \ and activate (Enter: collapse/restore; labelled copy.collapse/copy.expand) \u2014\
  \ the gesture alternative. RN View has no typed key handler, so hardware arrows,\
  ...
  \ child View overflowing the separator by max(handleSize, minTarget). The collapse\
  \ Button is a positioned sibling of the separator placed from its measured position,\
  \ not a child (Android drops touches outside a parent's bounds). persistKey is a\
  \ module-level memory map."
```
