...
notes: "A <div> of Disclosures rendered with headingLevel and shared padding overrides;\
  ...
  \ Every forwarded binding reaches the child through its `overrides` prop, not through\
  \ a rule the accordion writes into the child, so `--ds-accordion-item-gap` is the\
  \ only hook a consumer can set from CSS; the other six are settable only through\
  \ Accordion's own `overrides`."
```
