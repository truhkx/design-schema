...
notes: "Block element with max-width, margin-inline auto (or 0 for align start) and\
  ...
  \ known limit. The block `element: div` is the default; the root renders whichever\
  \ tag the `element` prop chooses. The root is the `column` part and carries `data-part=\"\
  column\"`, as Box's root carries `surface`."
```
