...
notes: "Native elements carry the roles: banner\u2192header, navigation\u2192nav,\
  ...
  \ warning; an unlabelled `region` with `as` set to another element follows the `as`\
  \ rule (role=\"region\" is emitted) and still warns. Landmark takes no className\
  \ or style (the Link rule): a composite that needs its own classes, style or positioning\
  \ (SidePanel) renders its own styled element and composes Landmark inside it, passing\
