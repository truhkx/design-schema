...
notes: 'Drawn with Views and a PanResponder per thumb (no new dependency; the community
  ...
  platform''s Actions menu. A pointer-driven control has no blur, so `validate: blur`
  commits at the end of an interaction (the drag release or the key-up), which is
  this field''s equivalent.'
```
