...
async function setup(given: Record<string, unknown> = {}) {
  ...
  const s = {
    ...
    props,
    root_: () => el,
    label: () => ((el.matches('[role="combobox"]') ? el : null) ?? deep(root, '[role="combobox"]') ?? (el.matches('[part~="label"], [data-part="label"]') ? el : null) ?? deep(root, '[part="label"]') ?? deep(root, '[data-part="label"]') ?? root.firstElementChild) as HTMLElement,
    chipRemove: () => ((el.matches('[part~="chipRemove"], [data-part="chipRemove"]') ? el : null) ?? deep(root, '[part="chipRemove"]') ?? deep(root, '[data-part="chipRemove"]')) as HTMLElement,
    ...
    toggleButton: () => ((el.matches('[part~="toggleButton"], [data-part="toggleButton"]') ? el : null) ?? deep(root, '[part="toggleButton"]') ?? deep(root, '[data-part="toggleButton"]')) as HTMLElement,
  };
  return s;
}
...
describe('ds-combobox', () => {
  ...
  test('the-toggle-button-opens-the-list', async () => {
    const s = await setup({"open": false});
    await userEvent.click(...(await pointerAt(activatable(s.toggleButton(), s.el), s.toggleButton())));
  ...
  test('multiple-shows-the-selection-as-chips', async () => {
    ...
    expect(flatText(s.el)).toMatch(new RegExp("Apple"));
  });
  test('removing-a-chip-reports-the-new-value', async () => {
    const s = await setup({"open": false, "multiple": true, "defaultValue": ["apple"]});
    await userEvent.click(...(await pointerAt(activatable(s.chipRemove(), s.el), s.chipRemove())));
    expect(s.events.onChange).toHaveBeenCalled();
  });
  test('a-disabled-combobox-does-not-open', async () => {
    const s = await setup({"open": false, "disabled": true});
    await userEvent.click(located(activatable(s.toggleButton(), s.el)), { force: true });
