...
- name: truncated-text-keeps-the-full-string-reachable
  description: Truncation clips to one line, and on web the full text is exposed via
    title when children is a plain string, so sighted users can also reach it.
  given:
    truncate: true
    children: A sentence long enough to be clipped by its column.
  then:
...
- truncate
- element
notes: "Renders the chosen element inside the shadow root with `part=\"text\"`; the\
  \ host is `display: contents` for `span`-like use and `display: block` otherwise.\
  \ `element` reflects so those two rules are `:host([element=\"span\"])` selectors\
  \ rather than an inline style \u2014 an inline `display: contents` would beat `:host([hidden])\
  \ { display: none }` and a hidden span would stay visible."
```
