...
notes: "A FlatList (virtualised \u2014 long option lists are common) of Pressable\
  ...
  \ Each option is its own accessibility stop; Pressable has no key events, so arrows,\
  \ Home/End, Page keys, typeahead, Shift+Arrow and Ctrl+A have no native form at\
  \ all, a tap is the selection, and `selectionFollowsFocus` is accepted for parity\
  \ with no runtime effect. `initialActiveValue` only pre-highlights a row here, since\
  \ there is no single tab stop to move."
```
