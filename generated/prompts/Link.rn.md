...
component:
  ...
  styles:
    ...
    colorHover:
      ...
      state: hover
      description: Pointer hover only on web and Lit (not :active); on native, which
        has no hover, the pressed color.
      locked: true
    colorVisited:
  ...
  platforms:
    web:
      ...
      notes: 'A native <a href>. `external` sets target="_blank" and rel="noopener
        ...
        its part on a wrapper element it owns.'
    lit:
      tag: ds-link
      reflect:
      - tone
...
notes: 'Renders Text with accessibilityRole="link" so it is inline inside a parent
  ...
  state; the pressed state uses colorHover. On react-native-web this becomes a real
  anchor. Forwards `accessibilityHint`, `accessibilityLabel` (when set by a parent
