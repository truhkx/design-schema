...
notes: 'A container with CSS grid (grid-template-columns: minmax(paneMinTarget, var(--ds-splitter-primary-size))
  ...
  in tab order. Below the stackBelow width a container query switches to a single
  column, the separator is not rendered, and both panes render in full. persistKey
