...
- Landmark: the RN notes say the label is dropped silently on `complementary` and `search`, but the guidance still asks the story set to give them labels ("Related links", "Site search"). On RN those stories carry a label nothing renders. I set them anyway for parity with web and added a comment in the stories.
- Landmark: the derived `renders-role-*` scenarios give only `role`, so taken literally they inherit Default's label "Main". The guidance says per-role scenarios use the per-role labels (and `undefined` for banner, main and contentinfo), which contradicts the scenario YAML. I followed the guidance, and the scenario `given` blocks should list those labels explicitly.
- Landmark: scenario `a-label-is-dropped-on-a-role-that-refuses-one` expects `accessibilityLabel` `is: null`, but on RN an unset prop is `undefined`, not `null`. I read `null` as 'absent' and asserted `toBeUndefined()` on both `accessibilityLabel` and `aria-label`. The doc should define `is: null` as absent.
- Landmark: the package rule to mirror `accessibilityLabel` as `aria-label` is not in the rn `platforms.props` list (only `role` and `accessibilityLabel` are). I added `aria-label` under the same role gate (navigation, region, form). The props list should name it.
- Landmark: the schema has no `overrides` (no style bindings) and no Keyboard story is needed (no keyboard block), so the component takes no `overrides` prop. The generic rule says `overrides` applies to every component, so the doc should say that a component with no bindings has no `overrides` prop.

## 2026-09-23 13:57 — round 1

- Landmark: platform notes say strings are wrapped 'at the top level only' and a raw string in an inner array is the caller's to wrap, but they also prescribe React.Children.map, which flattens nested arrays (not Fragments), so strings inside an inner array do get wrapped. Chose React.Children.map as prescribed; the doc should say whether inner arrays count as top level.
- Landmark: scenario a-label-is-dropped-on-a-role-that-refuses-one expects accessibilityLabel `is: null`, but on RN an unset prop is `undefined`, not `null`. The test asserts toBeUndefined; the doc could say 'absent' instead of null.
