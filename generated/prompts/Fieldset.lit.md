...
notes: Shadow root with a <fieldset><legend> and a default slot for the fields, which
  ...
  (the description, fields and errorMessage wrappers are Fieldset-owned <div>s carrying
  both data-part and part, as on web), so legendSize, legendWeight, helperSize, fontFamily
  and lineHeight reach them only as Text overrides, never as Fieldset rules or by
  setting --ds-text-* hooks. `disabled` is set as the `disabled` property on direct
  light-DOM children carrying data-ds-field, on slotchange and whenever `disabled`
  changes, remembering which it set so clearing never enables a field disabled on
  its own. The required indicator is derived from those same direct data-ds-field
  children's `required`, recomputed on slotchange and on their `required` attribute
  changes. The group's role and accessible name come from the native <fieldset>/<legend>
  inside the shadow root; ids never cross the shadow boundary.
```
