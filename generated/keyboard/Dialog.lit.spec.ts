// Generated by tools/keyboard_tests.ts from the `keyboard` block of the Dialog doc. Do not edit.
...
test.describe('Dialog (lit) keyboard', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/iframe.html?id=dialog-lit--keyboard&viewMode=story');
    await expect(page.getByRole('dialog').first()).toBeVisible({ timeout: 15_000 });
  });
  test('Escape: Requests close with reason escape (even when not dismissible).', async ({ page }) => {
    const root = await subject(page, page.getByRole('dialog').first());
  ...
  test('Tab: Moves to the next focusable element inside the dialog.', async ({ page }) => {
    const root = await subject(page, page.getByRole('dialog').first());
  ...
  test('Tab: From the last element, wraps to the first.', async ({ page }) => {
    const root = await subject(page, page.getByRole('dialog').first());
