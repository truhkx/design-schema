...
naming:
  ...
  aliases:
    # The names Demo Brand's product code already uses. Demo Brand shipped v2 calling it `ActionButton`,
    # with a `kind` prop whose main value was `cta`, and every screen written since imports exactly that.
    # The renames above are the vocabulary the brand is moving *to*; these keep the old one compiling while
    # the codebase moves, each one marked deprecated from the version the new names arrived in.
    #
    # Keys are canonical, like every other map here, so a pull that moves `Button` strands them loudly. The
    # names are the old spellings of the *current* names: `kind` is the old `emphasis`, `cta` the old
    # `primary`. tools/naming.ts writes them into `naming-compat/` when it applies the rename, and a revert
    # deletes that folder — nothing here is generated by the model, so nothing here is gated or hashed.
    components:
      Button:
        - name: ActionButton
          since: '2.0.0'
...
The compatibility layer is the same kind of edit. `pnpm demo:naming` writes
`packages/react/demo-brand/src/naming-compat/` from the `aliases` above — `CtaButton.ts`, a wrapper
that still takes `kind` and `cta` and exports `ActionButton`, and an `index.ts` barrel — and the
generator never adds that folder to a package's own `index.ts` or `exports`. Exposing it
(`@demo/react/naming-compat`, or a re-export from the package root) is Demo Brand's own manifest edit,
like the package scope.
