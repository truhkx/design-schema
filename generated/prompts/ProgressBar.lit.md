...
notes: "role=\"progressbar\" and aria-valuenow/min/max/text are plain reflected attributes\
  ...
  \ child. `formatValue` is a function, so it is property-only (`attribute: false`)\
  \ and unreachable from static HTML: a plain-HTML author gets the default percentage,\
  \ and the Lit story that shows a custom formatter uses a `.formatValue` property\
  \ binding."
```
