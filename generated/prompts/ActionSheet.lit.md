...
notes: '`actions` is a property. Composed `action` (detail { id }) and `close` (detail
  ...
  breakpoint media query is built from the theme token (maxWidth is locked). The forwarded
  ref resolves to the shadow <dialog>, null while closed. The heading is named by
  `aria-label` rather than an id reference, since ids do not cross the shadow root.'
```
