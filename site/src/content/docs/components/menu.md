...
component:
  ...
  props:
    ...
    placement:
      ...
      description: 'Preferred position of the popup relative to the trigger (or `anchor`). Only the block side flips (bottom and top swap) when the popup would overflow the viewport; `start` and `end` never flip. They resolve against the layout direction (in right-to-left `start` is the right edge; rn reads I18nManager.isRTL), and the popup is shifted inline instead so it stays `gutter` away from the side edges.'
    open:
      type: boolean
  ...
  keyboard:
    ...
    - { keys: [Escape], action: 'Closes and returns focus to the trigger; pressed on the trigger while the menu is open it also closes (reason `escape`) and focus stays there.', when: menu open, from: inside, expect: [closes, focus-trigger] }
  ...
  styles:
    ...
    typeaheadReset: { token: motion.duration.loop, part: popup, description: 'How long typed characters accumulate before the typeahead buffer clears; read at runtime from the popup.' }
    maxHeight: { token: layout.maxWidth.prose, part: popup, description: 'The popup''s block size never exceeds the viewport height minus `gutter` at each edge (2 × gutter); beyond that the list scrolls (the token is the cap used on wide screens).' }
    gutter: { token: layout.gutter, part: popup, description: 'Space kept between the popup and each viewport edge: it caps maxHeight (viewport height − 2 × gutter) and bounds the inline shift. Read in px the same way as popupOffset.' }
    ...
    layer: { token: layer.dropdown, part: popup, description: 'z-index of the position: fixed popup (web portal, Lit fallback); it has no effect inside the browser top layer (Lit popover="manual") or a native Modal window.' }
