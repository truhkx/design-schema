...
async function setup(given: Record<string, unknown> = {}) {
  ...
  const s = {
    ...
    props,
    root_: () => el,
    scrim: () => ((el.matches('[role="dialog"]') ? el : null) ?? deep(root, '[role="dialog"]') ?? (el.matches('[part~="scrim"], [data-part="scrim"]') ? el : null) ?? deep(root, '[part="scrim"]') ?? deep(root, '[data-part="scrim"]') ?? root.firstElementChild) as HTMLElement,
...
describe('ds-dialog', () => {
  ...
  test('initial-focus-lands-on-the-close-button', async () => {
    ...
    expect(activeChain()).toContain(focusedPart(s.closeButton(), s.el));
  });
