...
component:
  ...
  styles:
    ...
    labelColor: { token: color.foreground.muted, part: label, description: 'Locked, and deliberately not enforced by Divider: the colour arrives from the composed Text''s `tone: muted`, which resolves to this same token, and writing a colour rule here would restyle a composed child. The contrast pair is therefore a claim about Text''s muted tone on the page background, which no Divider code can regress.' }
