...
notes: "Native platforms have no landmark navigation (VoiceOver and TalkBack have\
  ...
  \ and `accessibilityLabel` props on the View instead of a role lookup. The `region`\
  \ part has no testID of its own: the root View is that part and keeps `testID=\"\
  Landmark\"`, as the Lit host keeps `data-ds` alone. Strings and numbers are wrapped\
  \ at the top level only \u2014 that is the contract, not a shortcut \u2014 so a\
  \ raw string nested inside a Fragment or an inner array is the caller's to wrap.\
  \ There is deliberately no module-level registry of mounted Landmarks, so the duplicate-main\
  \ and duplicate-label checks do not run on native: a registry would outlive unmounted\
  \ screens and report duplicates that are not on screen together. Nested-landmark\
  \ nesting rules are guidance only and are checked on no platform."
```
