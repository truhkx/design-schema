...
notes: "A View with `role=\"group\"` (not the legacy accessibilityRole) that is NOT\
  ...
  \ the legend text and toHaveAccessibleName on the group view; the name check lives\
  \ in the derived accessible-name scenario, so `the-legend-names-the-group` checks\
  \ text only on native. The composition's `element: span` is not passed to the Texts\
  \ (RN Text has no `element` prop). The visible legend Text includes copy.requiredIndicator\
  \ when shown, but FieldsetContext carries the bare legend, so fields read \"Shipping\
  \ address, Street\". The group error is announced as in Input: the Android live\
  \ region sits on the `Fieldset.errorMessage` wrapper View (role=alert has no native\
  \ equivalent), which is why `a-group-error-is-announced` is web/Lit only."
```
