...
- Breadcrumb: the package rule says to mirror accessibilityState.selected as aria-selected, but aria-selected is not allowed on the generic element react-native-web renders for a Text (axe aria-allowed-attr). The web contract uses aria-current="page", so I mirrored the current page with aria-current="page" instead, passed as an untyped attribute bag because RN Text's types lack aria-current. The rn notes should say which attribute mirrors `selected` here.
- Breadcrumb: the current page's accessibilityLabel / aria-label ('<label>, current page') lands on a generic span in react-native-web, where axe can report aria-prohibited-attr. The spec requires the label, so I kept it; the doc should say whether that axe rule is expected on web.
- Breadcrumb: `collapse` is a boolean, not an enum, so the rule of one story per enum value doesn't apply. I kept CollapseTrue and CollapseFalse as notable-state stories, and CollapseTrue duplicates the DeepTrailCollapsed example. The doc could say whether boolean props get stories named <Prop><Value>.
- Breadcrumb: the spec says focus after expanding goes to the index-1 item View through setAccessibilityFocus, but not whether that View must be `accessible` to receive focus. An inaccessible View may pass focus to its child, or ignore it, depending on the platform. I left the item View non-accessible so the Link inside stays separately focusable.
- Breadcrumb: the item-type annotation (`export type BreadcrumbItem = { label: string; href?: string | undefined }`) follows exactOptionalPropertyTypes, while the schema shape is written the same way. That matched, so no gap there, but the schema does not say whether `items` should be typed readonly; I kept the plain mutable array.

## 2026-09-23 13:58 — round 1

- Breadcrumb: the package rule says mirror every accessibilityState as aria-* (aria-selected for the current page), but aria-selected is not allowed on a generic element and the web contract uses aria-current="page"; chose aria-current="page" (spread through an untyped record, since RN's Text types omit aria-current) plus aria-label, and no aria-selected. The rn platform notes should say which ARIA mirror the current page takes.
- Breadcrumb: the rn notes say focus after expanding goes to the index-1 item View through setAccessibilityFocus, but a plain View that is not `accessible` may not be a screen-reader focus target on iOS/Android; kept the View as specified without adding accessible={true}, which would merge the item's children into one element and hide the Link inside it. The doc should say whether the item View is accessible.
