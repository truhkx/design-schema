...
component:
  ...
  status: review
  apg: dialog-modal
  anatomy: [scrim, surface, focusScope, handle, header, title, body, footer, closeButton]
  composition:
  ...
  props:
    ...
    title:
      type: string
      required: true
      description: The sheet's title and accessible name. May be visually hidden with `hideTitle` when the content is self-explanatory (a share sheet).
    hideTitle:
      type: boolean
      default: false
      description: Keep the title for assistive technology but do not render it.
    ...
    draggable:
      type: boolean
      default: true
      description: 'Drag the handle (or the sheet) downward to dismiss, with a velocity threshold. Purely additive: the close button and Escape always exist.'
  ...
  styles:
    ...
    maxWidth: { token: layout.maxWidth.prose, description: 'Above this viewport width the sheet renders as a centered Dialog of size md instead of rising from the edge.' }
  ...
  platforms:
    ...
    lit:
      tag: ds-bottom-sheet
      reflect: [open, height, dismissible, draggable]
...
### Lit
`<ds-bottom-sheet open title="Filters" height="half">`; shadow `<dialog>`; `matchMedia` decides presentation and re-renders on change; drag handling as web. Composes `<ds-heading>`, `<ds-button>`, `<ds-icon>`, `<ds-box>`, `<ds-stack>`, and `<ds-dialog>` for the wide presentation.
