...
notes: "An <article> when it has a heading (aria-labelledby the heading id), a <div>\
  ...
  \ adding a focus stop; the focus ring is drawn on the card via :focus-within. Card\
  \ reaches the child by cloning it with an extra class \u2014 the one sanctioned\
  \ exception to \"never touch a child\", because the hit area is the card's own geometry,\
  \ and both Link and Button merge a passed className by contract. The schema's a11y.role\
