...
notes: "role=\"status\" | \"alert\" from `live` (each implies its aria-live; set only\
  ...
  \ forwarded to the root. On dismiss, \"next focusable\" means a[href], button, input,\
  \ select, textarea, [tabindex] \u2265 0 or contenteditable in document order after\
  \ the alert, skipping any element with a negative tabindex, disabled elements (`:disabled`,\
  \ which includes descendants of a disabled fieldset), elements inside `inert`, and\
  \ elements that are not rendered: the `hidden` attribute or computed `display: none`\
  \ on the element or an ancestor, or computed `visibility: hidden`; size and layout\
  \ are not checked (jsdom has none)."
```
