...
2026-09-21 ActionSheet rn: focus-restore has no opener ref on native → stated as a native limit with its alternative
2026-09-21 ActionSheet rn: menu role on the surface swallowed heading and Cancel → role and name move to the list, as web
2026-09-21 ActionSheet rn: heading announced twice, as name and as text → accepted; the visible title stays exposed
2026-09-21 ActionSheet rn: one divider part covers two dividers → both keep the name, addressed in DOM order
2026-09-21 ActionSheet rn: no trigger, yet Keyboard must render one → only Keyboard renders a "More actions" trigger
2026-09-21 ActionSheet rn: nothing sizes the row glyph → new itemIconSize binding on font.size.md
2026-09-21 ActionSheet rn: who groups the danger actions last → the component does, whatever the array order
2026-09-21 ActionSheet rn: the list's scrolling and pinning unstated → the list is the scroll region, header and Cancel pinned
2026-09-21 ActionSheet rn: does a sheet mounted open animate in → yes, as BottomSheet
2026-09-21 ActionSheet lit: when focus returns under showModal → at the end of the exit, scope inactive from its start
2026-09-21 ActionSheet lit: layer names surface but is inert there → written on the <dialog>
2026-09-21 ActionSheet lit: which element the title hook targets → the composed Text node, from the sheet's stylesheet
2026-09-21 ActionSheet lit: no header leaves no top padding → the first row's itemPaddingBlock is the top spacing
2026-09-21 ActionSheet lit: Enter/Space expect closes but fires only → activation only; the story's consumer closes
2026-09-21 ActionSheet web: keyboard gate runs at a desktop viewport → it asserts the wide Menu model, stated
2026-09-21 ActionSheet web: which opener a presentation switch anchors to → captured once, the moment open turns true
2026-09-21 ActionSheet web: window blur maps to a scrim tap → a window blur reports nothing
2026-09-21 ActionSheet web: unresolvable breakpoint token → the sheet, retried while the document loads
2026-09-21 ActionSheet web: showModal exists but throws → catch it and fall back to the attribute
2026-09-21 ActionSheet web: velocity from a single move sample → 0, so distance alone decides
2026-09-21 ActionSheet web: unresolvable dragSlop → 0, as BottomSheet
2026-09-21 ActionSheet web: no enum props, so no enum stories → Closed and DismissibleFalse
2026-09-21 ActionSheet web: blank example args impossible in CSF → examples restate every prop they rely on
2026-09-21 ActionSheet web: does the danger icon carry meaning → decorative; label and grouping carry it
2026-09-21 ActionSheet web: ref typed non-null but null while closed → stated in the web notes
2026-09-21 AlertDialog web: icon mechanics contradict "As in Alert and Toast" → the analogy dropped, the mechanics stand
2026-09-21 AlertDialog web: layer is a silent no-op everywhere → a forward-compatibility hook; rn writes zIndex
2026-09-21 AlertDialog web: no scroll region for overflowing content → the surface scrolls; more content is a Dialog
2026-09-21 AlertDialog web: the browser closes the <dialog> anyway → report escape once, re-showModal, refocus Cancel
2026-09-21 AlertDialog web: inert-background outside showModal → showModal is the only mechanism, fallback non-modal
2026-09-21 AlertDialog web: scrim aria-hidden unspecified → aria-hidden on web and Lit
2026-09-21 AlertDialog web: no warning for empty required props → one dev warning naming all three
2026-09-21 AlertDialog web: is a Closed story wanted → yes, for parity with Dialog
2026-09-21 AlertDialog web: example stories cannot start blank → they restate their props explicitly
2026-09-21 AlertDialog lit: aria-description is Chromium-only → describe with aria-describedby inside the shadow root
2026-09-21 AlertDialog lit: is the trap armed through the exit → no, active follows open
2026-09-21 AlertDialog lit: footer gap prop versus the footerGap hook → the hook is the delivery path
2026-09-21 AlertDialog rn: initial focus on the title, not Cancel → stated per platform with the native alternative
2026-09-21 AlertDialog rn: gutter names surface, the wrapper is sized → stated: the animated wrapper carries the size
2026-09-21 AlertDialog rn: textGap has no part and no anatomy home → the unnamed titles column; inset covers the icon row
2026-09-21 AlertDialog rn: Tab and Enter have no native expression → stated, with the swipe order as the alternative
2026-09-21 BottomSheet web: footer Stack dropped Dialog's wrap → wrap: true in the composition
2026-09-21 BottomSheet web: no heading level anywhere → level 2, as Dialog
2026-09-21 BottomSheet web: focusScope part has no home → a sheet-owned element inside FocusScope
2026-09-21 BottomSheet web: unnamed scroll element around the body Box → named in partGap; the gap measures to it
2026-09-21 BottomSheet web: a browser-initiated close desyncs open → report escape once and re-showModal
2026-09-21 BottomSheet web: dragSlop units other than rem → px and unitless taken verbatim
2026-09-21 BottomSheet web: the drag is unreachable in jsdom → a finite clientY is required; browser-only proof
2026-09-21 BottomSheet web: velocity from one move sample → 0
2026-09-21 BottomSheet web: dismissDistance of which height → the surface's measured height at release
2026-09-21 BottomSheet web: release after open already went false → spring back, fire neither event
2026-09-21 BottomSheet web: overrides silently dead above the breakpoint → stated in the Behavior section
2026-09-21 BottomSheet web: non-dismissible Escape repeats → not throttled, every press reports
2026-09-21 BottomSheet lit: aria-label and aria-labelledby contradict → per-platform delivery stated once in Accessibility
2026-09-21 BottomSheet lit: is the headerless case web-only → no, every platform
2026-09-21 BottomSheet lit: heading tabindex -1 always or as fallback → only when it is the focus target
2026-09-21 BottomSheet lit: the 90% content cap has no token → contentCap constant (ratio, literal-ok)
2026-09-21 BottomSheet rn: SafeAreaView takes a partGap of its own → the gap above it is cancelled
2026-09-21 BottomSheet rn: rn Button accepts no hitSlop → the wrapper is sized, Button's own slop covers it
2026-09-21 BottomSheet rn: gestureState.vy versus sampled timestamps → timestamps, never vy
2026-09-21 BottomSheet rn: pinning the footer at height content → only a fixed height leaves slack to pin against
2026-09-21 BottomSheet rn: surface and focusScope testIDs → the surface carries the root, focusScope none
2026-09-21 BottomSheet rn: Tab rules read as if rn implemented them → stated as react-native-web only
2026-09-21 Dialog lit: gutter caps the <dialog>, pulling the scrim in → it caps the surface
2026-09-21 Dialog lit: widthMd computed against the raw token → the rule multiplies the hook
2026-09-21 Dialog lit: two redundant routes for the inset forward → the hook is the only route; rn passes the token path
2026-09-21 Dialog lit: the description-part note contradicts itself → wrapper kept, the rationale corrected
2026-09-21 Dialog lit: are example field values controlled → uncontrolled initial values
2026-09-21 Dialog lit: a Lit property cannot be required → defaults plus the dev warning, stated
2026-09-21 Dialog lit: reflect mixes prop and attribute names → initialFocus written out as a pair
2026-09-21 Dialog lit: onOpened payload void or empty object → dispatched with no detail at all
2026-09-21 Dialog lit: focus-visible ring on programmatic focus → intended; keyboard-opened dialogs show it
2026-09-21 Dialog lit: story parity added Closed, dropped NoFooter → one story list for every platform
2026-09-21 Dialog rn: layer says rn ignores it but siblings apply it → applied as zIndex, inert in a Modal
2026-09-21 Dialog rn: the scrim is described as two elements → Animated.View with a Pressable inside it
2026-09-21 Dialog rn: overflow hidden would clip the surface shadow → shadow and radius on the animated wrapper
2026-09-21 Dialog rn: bottom spacing with no footer → the surface column's own block padding
2026-09-21 Dialog rn: description has no native aria-describedby → accessibilityHint on the surface
2026-09-21 Dialog rn: Tab rules have no native expression → stated; only Escape survives
2026-09-21 Dialog rn: target-24px on a composed close button → Button's own size is the floor
2026-09-21 Dialog rn: swiftui notes cite a size value that does not exist → .fullScreenCover is for lg on compact width
2026-09-21 Menu web: roving state versus DOM focus when they diverge → DOM focus is authoritative, the index follows
2026-09-21 Menu web: may a disabled item hold focus → yes; arrows go to the first or last enabled item
2026-09-21 Menu web: the popup's name is specified twice → labelledby with a trigger, aria-label with anchor and on Lit
2026-09-21 Menu web: itemHover carries hover but must not transition → itemHover is exempt, stated
2026-09-21 Menu web: popup wider than viewport minus two gutters → the leading edge wins
2026-09-21 Menu web: where the layout direction is read → from the trigger or anchor, not the page
2026-09-21 Menu web: is the anchor's width a floor → no floor; the × 2.5 rule stands
2026-09-21 Menu web: unresolvable typeaheadReset → clear the buffer after each keypress
2026-09-21 Menu web: which focus-on-close rule wins → the focus-still-inside override
2026-09-21 Menu web: should Escape stop bubbling → prevented and stopped in both places
2026-09-21 Menu lit: part removed from all but popup and list → intended, stated
2026-09-21 Menu lit: Tab leaves the roving index nowhere → a suppression state; the next open starts at the first item
2026-09-21 Menu lit: reflecting open would make every menu controlled → it reflects the controlled property only
2026-09-21 Menu lit: minWidth needs a width CSS can read → platform-private --ds-menu-trigger-width
2026-09-21 Menu lit: no Lit idiom for handing over an anchor → the ref directive, or set after first render
2026-09-21 Menu lit: Placement stories render closed and show nothing → they render open on every platform
2026-09-21 Menu lit: the Tab parking's own focusout reports twice → counted as the Tab, tab-out only
2026-09-21 Menu lit: unresolvable popupOffset and gutter → 0
2026-09-21 Menu rn: gutter clamps the inline axis only → the block edges are clamped too
2026-09-21 Menu rn: maxHeight is a width token used as a height → cap on the popup; the shared token stated as deliberate
2026-09-21 Menu rn: an end-placed popup jumps when it measures → held at opacity 0 until measured
2026-09-21 Menu rn: do inert bindings stay in a platform's union → yes, for parity, and stated as inert
2026-09-21 Menu rn: focus restore "only when focus is inside" → web and Lit only; rn restores unconditionally
2026-09-21 Menu rn: where anchor-mode focus returns on native → to the anchor View, which must be focusable
2026-09-21 Menu rn: an uncontrolled anchor menu can never open → documented, still no warning
2026-09-21 Menu rn: the enter slide direction reads both ways → it starts toward the trigger and settles away
2026-09-21 Menu rn: the backdrop was named a scrim → Menu.backdrop; a non-modal menu has no scrim
2026-09-21 Menu rn: statusBarTranslucent on a non-modal popup → kept, with the coordinate-space reason
2026-09-21 Popover web: closeButton part cannot sit on the Button → a popover-owned span, clicks forwarded
2026-09-21 Popover web: example stories cannot start blank → they restate every prop at its default
2026-09-21 Popover web: boolean state stories have no names → Modal, WithArrow, NotDismissible
2026-09-21 Popover web: flip-shift when neither side fits → flip only if the opposite fits, then clamp cross-axis
2026-09-21 Popover web: the corner-arrow approximation is unspecified → a plain 50% of the panel edge, stated
2026-09-21 Popover web: what counts as inside once close() moved focus → the body counts as inside
2026-09-21 Popover web: modal scroll lock has no mechanism → Dialog's class plus scrollbar-gutter
2026-09-21 Popover web: who wraps Tab in a modal panel → FocusScope; Popover adds no handler
2026-09-21 Popover lit: a panel wider than the viewport → maxWidth clamped through a new gutter binding
2026-09-21 Popover lit: no name and no warning on web or Lit → the development warning extends to both
2026-09-21 Popover lit: a reflected boolean cannot express undefined → open reflects the controlled property only
2026-09-21 Popover lit: trigger cardinality unenforceable → every platform warns in development
2026-09-21 Popover lit: the example's date label invented per platform → the literal 16 September 2026 is in the given
2026-09-21 Popover lit: does a wrapper component get its own wrapper → no, data-part goes on ds-focus-scope itself
2026-09-21 Popover lit: is the focusable walk shared API → yes, FocusScope's exported focusableIn
2026-09-21 Popover rn: the panel and the root both claim the testID → the panel carries it; closed and phone cases stated
2026-09-21 Popover rn: the ref target is contradictory → no ref prop on React Native
2026-09-21 Popover rn: getByRole needs an accessible panel → the panel is not accessible; assert its props
2026-09-21 Popover rn: forwarding enter and exit re-times the sheet → forwarded only when the caller overrode them
2026-09-21 Popover rn: the trigger-name chain skips accessibilityLabel → the chain now matches Button's
2026-09-21 Popover rn: no heading and no close button → the header row is not rendered
2026-09-21 Popover rn: the collision margin had no token → a new gutter binding on layout.gutter
2026-09-21 Popover rn: the arrow's two outer edges unnamed → named per facing edge
2026-09-21 Popover rn: nothing to position against on the first frame → held at opacity 0 until measured
2026-09-21 Popover rn: accessibilityViewIsModal versus touch interception → it follows modal; the split is stated
2026-09-21 Popover rn: headingLevel with no heading → unused, and no warning
2026-09-21 SidePanel lit: a11y.role none roots the keyboard gate wrongly → roleFrom: role, as Landmark
2026-09-21 SidePanel lit: accessible-name checked on a closed panel → the requirement is about the shown panel
2026-09-21 SidePanel lit: which state the Default story shows → closed, behind its trigger, on both platforms
2026-09-21 SidePanel lit: ds-link cannot mark the current page → stated per platform; Link's schema logged to CODE
2026-09-21 SidePanel web: the focusScope column is not directly inside FocusScope → the surface sits between in modal mode
2026-09-21 SidePanel web: closeButton part cannot sit on the Button → an overlay-owned span, clicks forwarded
2026-09-21 SidePanel web: how headingGap's default travels → the hook route, as inset
2026-09-21 SidePanel web: aria-controls points at an unmounted dialog → set only while that element exists
2026-09-21 SidePanel web: how the breakpoint is converted to px → probe-measured once; jsdom renders the overlay
2026-09-21 SidePanel web: scroll lock through FocusScope, which has none → SidePanel locks the root itself, as Dialog
2026-09-21 SidePanel web: the body's scroll wrapper is unnamed → named; partGap measures to it
2026-09-21 SidePanel web: the width cap's scrollbar claim is false → 100% verbatim, the claim corrected
2026-09-21 SidePanel web: does web ever raise action → never; the reason is for a consumer's handler
2026-09-21 SidePanel web: should non-SwiftUI platforms carry copy.expanded → yes, verbatim and unused
2026-09-21 SidePanel web: a press starting on the trigger, ending elsewhere → reports trigger; the outside listener excludes it
2026-09-21 SidePanel rn: no ref even where a sidebar could have one → none in either mode, stated
2026-09-21 SidePanel rn: the parts column is unaddressable → testID SidePanel.focusScope, in both modes
2026-09-21 SidePanel rn: partGap's part does not exist when persistent → the sidebar root View is that column
2026-09-21 SidePanel rn: the edge swipe has no panel width → measured against the window width, stated
2026-09-21 SidePanel rn: exit assumes a controlled panel → uncontrolled flips in the same batch
2026-09-21 SidePanel rn: the hidden title has nowhere to move → the accessibilityLabel carries it, nothing renders
2026-09-21 SidePanel rn: trigger typed content but cloned → exactly one element, with a dev warning
2026-09-21 SidePanel rn: Escape has no native key → onRequestClose plus onAccessibilityEscape
2026-09-21 SidePanel rn: SafeAreaView deprecated but required → core SafeAreaView kept, the warning accepted
2026-09-21 SidePanel rn: is a narrow panel capped too → yes, all three widths keep the edgeGutter
2026-09-21 Accordion rn: exclusive trim warns or not, prose collides → the warning is about a declared value/defaultValue, never a trim
2026-09-21 Accordion rn: is keyboard dead in the reason union → kept for parity, never emitted, no rn keyboard scenario
2026-09-21 Accordion rn: do inherited parts get Accordion testIDs → no, list is the root, item is each Disclosure
2026-09-21 Accordion rn: do controlled opens and closes group → one pass in item order, interleaved
2026-09-21 Accordion rn: item disabled has no native effect stated → not pressable plus accessibilityState.disabled
2026-09-21 Accordion rn: keepMounted and headingLevel per item → accordion-wide, no per-item override
2026-09-21 Accordion web: forwarded bindings unreachable from CSS → only itemGap is a root hook, six go through overrides
2026-09-21 Accordion web: no copy key for the exclusive warning → wording is a dev aid, not contract; condition and frequency are
2026-09-21 Accordion web: value and defaultValue together → value controls, defaultValue ignored, no warning
2026-09-21 Accordion web: divider semantics unspecified → decorative, Divider's default semantic false
2026-09-21 Accordion web: where arrows are live, all-disabled case → triggers only, key left unhandled when all disabled
2026-09-21 Accordion web: unknown id, duplicate ids, items replaced → silently ignored, never pruned, no warnings
2026-09-21 Accordion lit: which form the keyboard gate exercises → the items form, matching React's story args
2026-09-21 Accordion lit: change collides with a field's composed change → both reach the page, detail.openIds identifies ours
2026-09-21 Accordion lit: items-mode item address undocumented → shadowRoot query, the child selector is slotted-only
2026-09-21 Accordion lit: keep-mounted reflected but unlisted → added to platforms.lit.reflect
2026-09-21 Accordion lit: defaultValue has no story anywhere → new initially-open example on every platform
2026-09-21 Combobox rn: iconColor override or color prop → the Icon's overrides.color on every platform
2026-09-21 Combobox rn: valueType string[] unreachable natively → stated, comma-joined multiple, bare single
2026-09-21 Combobox rn: invalid without error renders nothing → copy.invalid, as Input
2026-09-21 Combobox rn: the status part has no binding → takes the description treatment, helperSize and descriptionText
2026-09-21 Combobox rn: no target floor for the inner input → new inputMinTarget binding on size.target.min
2026-09-21 Combobox rn: anchored popup as a focus-trapping Modal → an absolutely positioned sibling, no scrim
2026-09-21 Combobox rn: filter none has no active option → the match becomes the Listbox initialActiveValue
2026-09-21 Combobox rn: onOpenChange on blur and commit closes → fires for every close, so controlled open tracks
2026-09-21 Combobox rn: unchanged text measured against what → the text shown now, the prop when controlled
2026-09-21 Combobox lit: part wrappers around ds-button → data-part on the ds-button itself, as ds-search
2026-09-21 Combobox lit: aria-controls cannot reach a shadow listbox → points at the popup div
2026-09-21 Combobox lit: status part presentation unspecified → the web treatment, hidden and polite
2026-09-21 Combobox lit: announcement pending when the list closes → cleared and blanked at once
2026-09-21 Combobox lit: does no-clear suppress Escape-clears-text → yes, clearable gates both
2026-09-21 Combobox lit: what a cleared value reports → '' single, [] multiple
2026-09-21 Combobox lit: activeOption stale in aria-describedby → linked only while open with an active option
2026-09-21 Combobox lit: resultCount locale from inside a shadow root → light-DOM ancestors then the runtime default
2026-09-21 Combobox lit: input-change on an already-selected match → fires, like any other commit clear
2026-09-21 Combobox lit: chip truncation point undefined → no width binding, the label shrinks in the row
2026-09-21 Combobox web: does the open prop move DOM focus → yes, unless focus is already in the field
2026-09-21 Combobox web: dangling aria-controls while closed → always rendered, per APG, unlike Select
2026-09-21 Combobox web: empty-string error → not a message, as Input
2026-09-21 Combobox web: addCustom row with filter none → shown, suppression depends only on a match
2026-09-21 Combobox web: does a click in the input drop the filter → yes, the full list, like the toggle
2026-09-21 Combobox web: statusDebounce with no debounce → still asynchronous, a zero-delay timer
2026-09-21 Combobox web: Alt+ArrowDown default action when open → suppressed, the caret never moves
2026-09-21 Listbox rn: react-native-web gets list plus menuitem → emits the listbox contract and real ARIA attributes there
2026-09-21 Listbox rn: accessibilityState dropped by rnw → written as attributes, aria-disabled in an effect
2026-09-21 Listbox rn: no native carrier for the error → each row's accessibilityHint
2026-09-21 Listbox rn: do root type bindings reach composed Texts → fontFamily and lineHeight forward, fontSize does not
2026-09-21 Listbox rn: optionGap between label and description → the row's horizontal gap only
2026-09-21 Listbox rn: does the pre-highlighted row report on focus → every real row focus reports, only hover dedupes
2026-09-21 Listbox rn: does a disabled list pre-highlight → resolves it, draws nothing, reports nothing
2026-09-21 Listbox rn: submit focus versus virtualisation → scrollToIndex first, else the list View
2026-09-21 Listbox rn: selectedCount has one plural form → intended, "1 selected" is not special-cased
2026-09-21 Listbox rn: ListboxGroupOption deprecated alias → only where a package shipped the name
2026-09-21 Listbox rn: optionIcon has no colour binding → takes optionColor as the Icon's overrides.color
2026-09-21 Listbox lit: empty row inside a role=listbox → aria-hidden and linked by aria-describedby
2026-09-21 Listbox lit: tabindex 0 flat versus embedded → -1 when embedded, 0 otherwise, disabled included
2026-09-21 Listbox lit: two unqualified weight bindings → optionSelectedWeight takes state selected
2026-09-21 Listbox lit: no-focus test for a host-driven list → the list does not contain the active element
2026-09-21 Listbox lit: repeated letter versus the typeahead buffer → collapses to that letter, searches after the active one
2026-09-21 Listbox lit: PageUp by rows or by options → enabled options, never drawn rows
2026-09-21 Listbox lit: array or string against the mode → normalised both ways, no warning
2026-09-21 Listbox lit: ancestor-disabled through formDisabledCallback → counts as disabled, CSS keyed on aria-disabled
2026-09-21 Listbox lit: composed parts need id and element=p → both stated as exceptions to the listed props
2026-09-21 Listbox lit: borderInvalid missing from the embedded no-ops → added
2026-09-21 Listbox lit: option ids in a shadow root → instance and index scoped, hosts reach by data-value
2026-09-21 Listbox lit: a LabelledBy story for a web-only prop → keeps it, it shows the list still named by label
2026-09-21 Listbox lit: Default story args disagree across platforms → the single-picker example everywhere
2026-09-21 Listbox web: selection state read off the focused list → an activedescendant composite reports it on the item
2026-09-21 SegmentedControl rn: rnw drops accessibilityState and focusable → aria mirrors plus tabindex on the node
2026-09-21 SegmentedControl rn: segmentIcon colour unbound → its segment's own foreground, forwarded to the Icon
2026-09-21 SegmentedControl rn: locked minTarget wrong on touch → the binding resolves per platform, both tokens named
2026-09-21 SegmentedControl rn: size changes no height natively → stated as the intended outcome
2026-09-21 SegmentedControl rn: first pill placement after measurement → instant, only later moves slide
2026-09-21 SegmentedControl rn: group width when fill is off → hugs its segments
2026-09-21 SegmentedControl rn: shadow.raised is not a scalar → stated, expands to RN shadow style keys
2026-09-21 SegmentedControl rn: iconOnly fallback width → the label at its natural width, no reserved box
2026-09-21 SegmentedControl lit: how a disabled option is disabled → aria-disabled plus a click guard, never native disabled
2026-09-21 SegmentedControl lit: no Space or Enter in the table → added as a native rule on the underlying button
2026-09-21 SegmentedControl lit: which value reflect means → the property only, uncontrolled writes no attribute
2026-09-21 SegmentedControl lit: minTarget axis unstated → both, inline and block
2026-09-21 SegmentedControl lit: full-opacity pill under a faded label → intended, disabledOpacity names segment only
2026-09-21 SegmentedControl lit: Tooltip on a disabled icon-only segment → kept, the label stays discoverable
2026-09-21 SegmentedControl lit: empty options array → a documented no-op, no tab stop, no warning
2026-09-21 SegmentedControl web: which field the iconOnly warning lists → value, and the wording is not contract
2026-09-21 SegmentedControl web: two overlapping toolbar sentences → focus first, then the end-segment exception
2026-09-21 SegmentedControl web: toolbar lookup start and its test gap → from parentElement, stated as ungated guidance
2026-09-21 SegmentedControl web: iconOnly named twice → aria-label is primary, the Tooltip's is accepted
2026-09-21 SegmentedControl web: Tooltip owns the ref → segments found by generated DOM id
2026-09-21 SegmentedControl web: payload asserted on lit only → the with assertions now cover every platform
2026-09-21 SegmentedControl web: do the two padding bindings collapse → they stay independent hooks
2026-09-21 Select rn: rnw combobox missing required attributes → aria-expanded mirror, aria-controls and aria-disabled in an effect
2026-09-21 Select rn: Select.popup inside a BottomSheet → a layout-only View, and no scrim testID on phones
2026-09-21 Select rn: chevronReserve on a platform without it → inert but accepted, hook read only in the native wrap
2026-09-21 Select rn: what the trigger announces with no selection → the text it shows, the placeholder prop when given
2026-09-21 Select rn: Fieldset never mentioned → behaves like every other field, disabled and legend included
2026-09-21 Select rn: disabled versus a controlled open → forced closed, aria-expanded false, no event
2026-09-21 Select rn: instant close justified by the web top layer → stated as the cross-platform rule
2026-09-21 Select rn: two-label join not locale-aware → a literal comma and space, deliberately not Intl.ListFormat
2026-09-21 Select rn: initialActiveValue with several selected → the first value in array order
2026-09-21 Select rn: focus ring on a device → Pressable sees no focus there, so no ring at all
2026-09-21 Select rn: description announced twice → accepted, as Input
2026-09-21 Select rn: invalid without error has no scenario → the invalid scenario now asserts copy.invalid
2026-09-21 Select lit: labelledBy inert on Lit → passed for parity, the list is named by label
2026-09-21 Select lit: no Space rule while the popup is open → added to the keyboard table
2026-09-21 Select lit: where a toggled value lands → Listbox's own order, unknown values appended
2026-09-21 Select lit: how a story owns a controlled open → writes open-change back and focuses the trigger
2026-09-21 Select lit: forwards defeat the root CSS hooks → stated, the escape hatch is partial for the four
2026-09-21 Select lit: aria-controls always or while open → web while open, Lit always, the difference is deliberate
2026-09-21 Select lit: unhookable value clipping → stated as not overridable
2026-09-21 Select lit: focus-ring clamp forces a 0px literal → clamp dropped, Input's unclamped rule
2026-09-21 Select web: target-44px versus minTargetSm → 44px applies at md, sm is the documented exception
2026-09-21 Select web: error wrapper versus Input's shape → the composed Text carries role, id and part
2026-09-21 Select web: composition states a value twice → the forward is authoritative
2026-09-21 Select web: auto and never render the same → stated as expected, only always differs
2026-09-21 Tabs rn: rnw honours neither focusable nor state → tabindex and aria-disabled in an effect, aria-selected mirrored
2026-09-21 Tabs rn: disabled tab contrast can never pass → rests on the 1.4.3 exemption, which needs the state in the tree
2026-09-21 Tabs rn: tab stop pinned or following focus → follows the focused tab, returns on focus out
2026-09-21 Tabs rn: a type-only deprecated alias cannot warn → the JSDoc tag alone, only where the name shipped
2026-09-21 Tabs rn: a fill list that overflows → does not scroll, keep-in-view is a no-op
2026-09-21 Tabs rn: defaultValue naming a disabled or missing tab → honoured verbatim, the stop falls back
2026-09-21 Tabs rn: keepMounted panels share one testID → stated, tests use getAllByTestId
2026-09-21 Tabs rn: indicator edge is logical, RN is physical → the requirement is a measurement, each platform computes it
2026-09-21 Tabs rn: does keep-in-view scrolling animate → yes, unless reduced motion
2026-09-21 Tabs rn: do disabled tabs count in copy.position → yes, index and total
2026-09-21 Tabs rn: panel label on a non-accessible View → kept, the iOS limit recorded
2026-09-21 Tabs lit: RTL arrow swap lives only in prose → moved into the keyboard rules
2026-09-21 Tabs lit: tab stop after a programmatic focus → syncs to the focused tab
2026-09-21 Tabs lit: URL args dropped without an argType → stated for every platform with a keyboard block
2026-09-21 Tabs lit: unnamed state stories drifted → Controlled, FitFill, KeepMounted and TabWithoutPanel named with their args
2026-09-21 Tabs lit: badge line height assumes unitless → stated, the token is unitless in every theme
2026-09-21 Tabs web: indicator animation gate unnamed → the component gates it, data-animate on web, a latch on RN
2026-09-21 Tabs web: keep-in-view math is LTR-only → measured from the tab's offset, not a non-negative scrollLeft
2026-09-21 Tabs web: Enter and Space under automatic → no handler of ours, the native click no-ops
2026-09-21 Tabs web: tab name by content or aria-labelledby → content on web, labelledby on Lit, joined label on RN
2026-09-21 Tabs web: no focus binding for the panel → draws from the tab's focusRing hooks
2026-09-21 Tabs web: indicator with a selection matching no tab → renders at zero size
2026-09-21 Slider rn: react-native-web drops accessibilityValue and State → every adjustable writes the aria-* spellings too
2026-09-21 Slider rn: an accessibility action has no interaction end → each action fires onValueChange then onSlidingComplete
2026-09-21 Slider rn: press on a thumb versus on the track → the thumb's PanResponder wins, it drags from its value
2026-09-21 Slider rn: action names for page, home and end unstated → pageUp, pageDown, home, end beside increment and decrement
2026-09-21 Slider rn: hint and visible text both read the description → accepted, neither carrier can be dropped
2026-09-21 Slider rn: halo on a platform with no press state → shown while a gesture owns that thumb
2026-09-21 Slider rn: mark labels collide or overflow the ends → no collision handling, min and max labels hang past the track
2026-09-21 Slider web: disabledOpacity dims text with no disabled state → the dimming element carries aria-disabled as well
2026-09-21 Slider web: errorText override accepted or absent → locked means absent from the type
2026-09-21 Slider web: Home and End on a range thumb → the live constraint from the other thumb, never a cross
2026-09-21 Slider web: aria-valuetext duplicates valuenow by default → always emitted
2026-09-21 Slider web: validate blur predates submitFailed → change mode and post-failed-submit validate every commit
2026-09-21 Slider web: bubble under showValue always → the bubble element exists only under hover
2026-09-21 Slider web: markSize equals trackHeight, ticks read as bumps → markSize is space.2, proud of the rail
2026-09-21 Slider web: range mismatched with the value's shape → silent fallback to that mode's default, only max <= min warns
2026-09-21 Slider web: required for a range pair → a difference in either component satisfies it
2026-09-21 Slider lit: unions have no attribute converter → value and defaultValue are property-only, stated
2026-09-21 Slider lit: errorMessage part is the Text or a wrapper → the Text, inside an unparted role=alert div
2026-09-21 Slider lit: a consumer id has no shadow reading → the id stays on the host, the low thumb is always #thumb
2026-09-21 Slider lit: formStateRestoreCallback unspecified → one string, or two same-name entries for a range
2026-09-21 Slider lit: error is not in form.validation → reported as a custom validity, error then required then invalid
2026-09-21 Slider lit: keyboard table gives no right-to-left rule → ArrowLeft and ArrowRight mirror, ArrowUp and ArrowDown never
2026-09-21 Slider lit: snapToMarks with no marks → inert, drag falls back to the step grid
2026-09-21 Slider lit: max unreachable by drag on a partial step → the last partial step snaps to max
2026-09-21 Slider lit: one bubble described, a range has two → one per thumb, every bubble aria-hidden
2026-09-21 Slider lit: trackPaddingBlock cannot contain minTarget → the hit area overflows, the label rows take the press
2026-09-21 NumberInput web: aria-hidden around focusable steppers → dropped, steppers announced with their copy labels
2026-09-21 NumberInput web: invalid over a valid number reads wrongly → invalid alone draws no message
2026-09-21 NumberInput web: two precedences implied by required → stated, the rendered slot needs invalid or error
2026-09-21 NumberInput web: percent plus a trailingText percent sign → percent suppresses trailingText, as currency does leadingText
2026-09-21 NumberInput web: format unit with no unit prop → plain decimal, no suffix
2026-09-21 NumberInput web: aria-valuenow on an empty field → omitted, not an invented zero
2026-09-21 NumberInput web: hold-to-repeat beyond the timings → pointerdown steps, the click is swallowed, repeat stops at a bound
2026-09-21 NumberInput web: form.valueType number versus String(value) → valueType is the meaning, the string is the wire form
2026-09-21 NumberInput lit: borderFocus compensation across two parts → a custom property on the field, block on the input
2026-09-21 NumberInput lit: description part is the Text or a wrapper → the wrapper, which also carries the describedby id
2026-09-21 NumberInput lit: every keyboard rule is manual → stated, value-changing rules are gated by scenarios
2026-09-21 NumberInput rn: accessibilityElementsHidden over Buttons → dropped, only the affix Texts are hidden
2026-09-21 NumberInput rn: dimmed label fails contrast with no state → aria-disabled mirrored on the root View and TextInput
2026-09-21 NumberInput rn: how a Form marks a field invalid → an entry under the field's name, empty falls to derived copy
2026-09-21 NumberInput rn: onChangeText collides with TextInput's prop → the declared name stands, the payload is the number
2026-09-21 NumberInput rn: Enter outside a Form → commits and keeps focus, submitBehavior submit
2026-09-21 NumberInput rn: hideLabel leaves requiredIndicator homeless → it survives inside accessibilityLabel only
2026-09-21 NumberInput rn: bounds not announced from accessibilityValue → stated, the out-of-range copy carries them
2026-09-21 NumberInput rn: stepper target floor at size sm → the field's own 24px floor, the arrows are the 2.5.8 exception
2026-09-21 ProgressBar web: every announcement deferred a frame or the first → only the first, later ones are synchronous
2026-09-21 ProgressBar web: showValue under an invalid range → aria-valuetext always, the visible text obeys showValue
2026-09-21 ProgressBar web: malformed bounds with max below min → reported exactly as given, never swapped or clamped
2026-09-21 ProgressBar web: live region a sibling or a child → the last child of the root, so one element is returned
2026-09-21 ProgressBar web: a caller passing tabIndex → the root accepts none, it is never focusable
2026-09-21 ProgressBar lit: reduced-motion opacity had no binding → new indeterminateReducedOpacity on opacity.disabled
2026-09-21 ProgressBar lit: an unparseable value attribute → NaN is non-finite, so determinate at min
2026-09-21 ProgressBar lit: where hideLabel's hiding styles go → a wrapper span, the label part stays a clean Text
2026-09-21 ProgressBar lit: formatValue has no attribute form → property-only, the story binds .formatValue
2026-09-21 ProgressBar rn: accessibilityRole versus the role prop → role=progressbar, as Meter, plus the flattened aria-*
2026-09-21 ProgressBar rn: sweep start and end offsets unstated → from minus the sweep width to plus the track width, mirrored
2026-09-21 ProgressBar rn: fill before the track is measured → snaps on first layout, on resize and under reduced motion
2026-09-21 ProgressBar rn: how often the invalid-range warning fires → once per distinct min and max pair
2026-09-21 ProgressBar rn: a long label pushes the value out → the label wrapper shrinks, stated as layout
2026-09-21 ProgressBar rn: hideLabel with no value leaves no header → the header View is not rendered, stated as the exception
2026-09-21 ProgressBar rn: formatValue runs while indeterminate → not called then, nothing shows or announces
2026-09-21 ProgressBar rn: invalid range then a valid one → the silent record is re-armed each time the range is invalid
2026-09-21 ProgressBar rn: announcement rules have no scenarios → the gate boundary is stated, packages cover the machine
2026-09-21 Stepper web: minTarget on the list item or the control → the navigable control only, both keep the focus border
2026-09-21 Stepper web: connector centring formula's axis → per orientation, the padding only on the offset axis
2026-09-21 Stepper web: count spacing versus display none → the root's own gap, so it collapses with the count
2026-09-21 Stepper web: a11y.role none cannot be queried → the landmark is named, a11y.role is navigation
2026-09-21 Stepper web: an explicit upcoming status on the current step → legal, selected with no status word, discouraged
2026-09-21 Stepper web: compact with an unmatched current → compact reveals the first step's label
2026-09-21 Stepper web: indicatorFontSize drives an Icon size → intended, size=sm on those Icons is decorative
2026-09-21 Stepper lit: describedby on a non-navigable div → the pairing is button-only, plain text otherwise
2026-09-21 Stepper lit: order of label, status word and description → label, status word, description
2026-09-21 Stepper lit: a reflected default is absent before first update → horizontal rules are written as not vertical
2026-09-21 Stepper lit: vertical connector growth has no free space → growth is horizontal-only, vertical is exactly stepGap
2026-09-21 Stepper rn: role=list with button children → each step is a role=listitem View carrying the step testID
2026-09-21 Stepper rn: connector inside or beside the steps → a sibling on native, stated as the anatomy difference
2026-09-21 Stepper rn: fontFamily on a View that carries no text style → the indicator numeral plus every Text forward
2026-09-21 Stepper rn: which View the ref is → the plain root, list plus count, so it measures the whole component
2026-09-21 Stepper rn: compact set by hand on a vertical stepper → vertical never renders the count
2026-09-21 Stepper rn: the unmatched-current warning text → given verbatim in the doc, identical on every platform
2026-09-21 Stepper rn: status-word scenarios excluded from rn → stated why, the word lives in accessibilityLabel there
2026-09-21 Stepper rn: the focus ring is inset with no outline → stated, every control reserves the transparent border
2026-09-21 Search web: two close scopes for the list → one scope, outside Search, and Tab closes explicitly
2026-09-21 Search web: aria-controls points at nothing when closed → set only while the list is open, with activedescendant
2026-09-21 Search web: ArrowUp with no highlight → a no-op that neither opens nor alters the list
2026-09-21 Search web: focus after submit or a chosen suggestion → stays put, only clear returns it to the input
2026-09-21 Search web: an empty submit attempt and the open list → every submit attempt closes it
2026-09-21 Search web: what the live region holds once closed → cleared, the status text is only ever current
2026-09-21 Search web: Form-level disabled and registration → unregistered either way
2026-09-21 Search web: the Icon size mapping is not a binding → confirmed, the glyph tracks size and is not themeable
2026-09-21 Search web: the Keyboard story's args were a guess → named, invoices plus the with-suggestions list
2026-09-21 Search web: a conditional searchbox or combobox role → both written explicitly, combobox wins with suggestions
2026-09-21 Search lit: a hook for a forwarded binding reads nothing → no hook is defined, overrides is the only route
2026-09-21 Search lit: suggestionsOffset as a margin or a position → folded into the fixed position, as ds-combobox
2026-09-21 Search lit: no keyed remount to clear the highlight → activeValue = null on the ds-listbox
2026-09-21 Search lit: the composed Listbox's own events cross the boundary → stopped and never re-dispatched
2026-09-21 Search lit: two polite regions compete → the describedby span is the live one, the status region unlinked
2026-09-21 Search lit: where the plural locale comes from → the nearest lang ancestor, then the runtime locale
2026-09-21 Search lit: a controlled field and a chosen suggestion → the label is submitted, the field waits for the caller
2026-09-21 Search lit: the popup's inline size is not a binding → at least the field's width, a long label may widen it
2026-09-21 Search rn: searchbox is not an accessibilityRole → role=searchbox on the TextInput, search on the container
2026-09-21 Search rn: loading announced only with an open list → announced whenever suggestions is set and loading
2026-09-21 Search rn: which Listbox wiring survives on native → six props, no ids, no active change, no remount key
2026-09-21 Search rn: name does nothing on native → name is the Form key everywhere, only action is inert
2026-09-21 Search rn: dimmed text over an undimmed frame → the contrast pairs are the enabled state, aria-disabled carries it
2026-09-21 Search rn: the Tab rule has no native equivalent → stated with Escape and ArrowDown as hardware-keyboard only
2026-09-21 DatePicker web: Escape while focus is in the input → focus stays, the handler is DatePicker's own on the root
2026-09-21 DatePicker web: lineHeight cannot reach the helper Texts → stated, it does not apply to them
2026-09-21 DatePicker web: ArrowDown on an open calendar from the end input → the end date, as reopening does
2026-09-21 DatePicker web: the year format in the grid label → String(year), the same string the Select shows
2026-09-21 DatePicker web: which clock today reads → the device's local calendar date, everything else UTC
2026-09-21 DatePicker web: an unparseable min or max → ignored for the year range, still compared when disabling
2026-09-21 DatePicker web: container is not in the schema → a platform-only portal prop, stated
2026-09-21 DatePicker lit: data-part popover on a host that never hides → moved to the calendar content wrapper
2026-09-21 DatePicker lit: header anatomy order versus Tab order → the DOM follows the Tab order
2026-09-21 DatePicker lit: variant and size of five Buttons in a web note → stated once, ghost, sm, the trigger takes size
2026-09-21 DatePicker lit: the prev and next glyphs are unnamed → chevron-left and chevron-right
2026-09-21 DatePicker lit: the en dash has no part → decoration, reachable only through its binding
2026-09-21 DatePicker lit: focus restore under a controlled open → on the state change, not on the request
2026-09-21 DatePicker lit: required on the range's end field → inert, there for shape, the start reports the message
2026-09-21 DatePicker lit: weekdaySize and weekdayWeight have no part → part weekdayHeader, the week header cell inherits it
2026-09-21 DatePicker lit: the today ring's transition → border-color joins background-color and color on web and lit
2026-09-21 DatePicker lit: story parity states absent from examples → with-a-value, range-with-dates, with-an-error, german-locale added
2026-09-21 DatePicker rn: the grid's name has no native carrier → stated as a native limit, the month announcement is the path
2026-09-21 DatePicker rn: transition between two day fills → one fill layer's opacity, a colour change switches at once
2026-09-21 DatePicker rn: dayHover on a platform with no hover → web-only there, wired anyway
2026-09-21 DatePicker rn: the focused day has no ring binding → it borrows the root's focusRing at the today ring's geometry
2026-09-21 DatePicker rn: the grid's row count → always six rows, on every platform
2026-09-21 DatePicker rn: weekdayHeader singular or per cell → per cell, all seven, never the row
2026-09-21 DatePicker rn: open combined with disabled → user opens blocked, a controlled open still renders
2026-09-21 DatePicker rn: typing against a pending range draft → the draft is untouched until a pick, Clear or close
2026-09-21 DatePicker rn: the 3660-day skip limit on a platform with no arrows → marked web and lit only
2026-09-21 DatePicker rn: fieldGap around the range en dash → applied uniformly as the row's gap
