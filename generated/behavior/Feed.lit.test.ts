...
async function setup(given: Record<string, unknown> = {}) {
  ...
  const s = {
    ...
    props,
    root_: () => el,
    container: () => ((el.matches('[role="feed"]') ? el : null) ?? deep(root, '[role="feed"]') ?? (el.matches('[part~="container"], [data-part="container"]') ? el : null) ?? deep(root, '[part="container"]') ?? deep(root, '[data-part="container"]') ?? root.firstElementChild) as HTMLElement,
    newItemsButton: () => ((el.matches('[part~="newItemsButton"], [data-part="newItemsButton"]') ? el : null) ?? deep(root, '[part="newItemsButton"]') ?? deep(root, '[data-part="newItemsButton"]')) as HTMLElement,
  };
  return s;
}
...
describe('ds-feed', () => {
  ...
  test('pressing-show-new-asks-for-the-newer-items', async () => {
    ...
    await userEvent.click(...(await pointerAt(activatable(s.newItemsButton(), s.el), s.newItemsButton())));
  ...
  test('the-end-message-shows-when-there-is-nothing-more', async () => {
    ...
    expect(flatText(s.el)).toMatch(new RegExp("You\\ are\\ all\\ caught\\ up\\."));
  });
  test('a-custom-end-message-replaces-the-default', async () => {
    ...
    expect(flatText(s.el)).toMatch(new RegExp("That\\ is\\ everything\\ from\\ this\\ week\\."));
  });
  test('an-empty-feed-that-is-not-loading-says-so', async () => {
    const s = await setup({"items": [], "hasMore": false, "loading": false});
    expect(flatText(s.el)).toMatch(new RegExp("Nothing\\ here\\ yet\\."));
  });
