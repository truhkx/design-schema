...
async function setup(given: Record<string, unknown> = {}) {
  ...
  const s = {
    ...
    props,
    root_: () => el,
    trigger: () => ((el.matches('[role="complementary"]') ? el : null) ?? deep(root, '[role="complementary"]') ?? (el.matches('[part~="trigger"], [data-part="trigger"]') ? el : null) ?? deep(root, '[part="trigger"]') ?? deep(root, '[data-part="trigger"]') ?? root.firstElementChild) as HTMLElement,
    scrim: () => ((el.matches('[part~="scrim"], [data-part="scrim"]') ? el : null) ?? deep(root, '[part="scrim"]') ?? deep(root, '[data-part="scrim"]')) as HTMLElement,
    closeButton: () => ((el.matches('[part~="closeButton"], [data-part="closeButton"]') ? el : null) ?? deep(root, '[part="closeButton"]') ?? deep(root, '[data-part="closeButton"]')) as HTMLElement,
  };
  return s;
}
...
describe('ds-side-panel', () => {
  test('close-button-fires-on-open-change', async () => {
    ...
    await userEvent.click(...(await pointerAt(activatable(s.closeButton(), s.el), s.closeButton())));
  ...
  test('non-dismissible-scrim-tap-does-nothing', async () => {
    const s = await setup({"open": true, "dismissible": false});
    await userEvent.click(...(await pointerAt(activatable(s.scrim(), s.el), s.scrim())));
  ...
  test('the-heading-is-rendered', async () => {
    ...
    expect(flatText(s.el)).toMatch(new RegExp("Your\\ cart"));
  });
  test('renders', async () => {
    const s = await setup({"open": true});
  ...
  test('has-accessible-name', async () => {
    const s = await setup({"open": true});
    if (hostName(s.el) === null) expect(s.trigger()).toHaveAccessibleName();
  });
